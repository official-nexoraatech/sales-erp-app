    ...
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow">
        <div className="flex items-center justify-between border-b px-5 py-4">
          ...
          <Button onClick={() => navigate('/purchase/payment-out/create')} className="min-w-[170px]">Create Payment Out</Button>
        </div>
        ...
        <div className="overflow-x-auto px-3 pb-3">
          {payments.isLoading ? <div className="p-10"><Loader /></div> : <table className="w-full text-sm">
            <thead className="bg-gray-50"><tr><th className="border p-3"><input type="checkbox" checked={allSelected} onChange={() => setSelectedIds(allSelected ? [] : rows.map((payment) => payment.paymentId))} /></th>{exportColumns.concat('Action').map((heading) => <th key={heading} className="border p-3 text-left">{heading}</th>)}</tr></thead>
            <tbody>{rows.length ? rows.map((payment: PaymentListItem) => <tr key={payment.paymentId} className="border-b even:bg-gray-50"><td className="border p-3"><input type="checkbox" checked={selectedIds.includes(payment.paymentId)} onChange={() => setSelectedIds((current) => current.includes(payment.paymentId) ? current.filter((id) => id !== payment.paymentId) : [...current, payment.paymentId])} /></td><td className="border p-3">{formatDate(payment.paymentDate)}</td><td className="border p-3">{payment.paymentNo || 'N/A'}</td><td className="border p-3">{payment.paymentNo || 'N/A'}</td><td className="border p-3">{payment.supplierName || payment.partyName || 'N/A'}</td><td className="border p-3 font-semibold">{formatCurrency(payment.amount)}</td><td className="border p-3">{user?.userName || 'admin'}</td><td className="border p-3">{formatDate(payment.paymentDate)}</td><td className="border p-3"><button onClick={() => navigate(`/purchase/payment-out/${payment.paymentId}`)} className="text-blue-600"><Eye size={17} /></button></td></tr>) : <tr><td colSpan={9} className="bg-gray-50 p-5 text-center">No data available in table</td></tr>}</tbody>
          </table>}
        </div>

...
export const StockAdjustmentListPage: React.FC = () => {
  ...
  const download = (extension: 'csv' | 'xls') => {
    ...
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `stock-adjustments.${extension}`; link.click(); URL.revokeObjectURL(url);
  };
  const printPdf = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow">
        <div className="flex items-center justify-between border-b px-5 py-4"><h1 className="text-xl font-semibold uppercase text-gray-900">Stock Adjustment List</h1><Button onClick={() => navigate('/stock/adjustments/create')} className="min-w-[170px]">New Adjustment</Button></div>
        ...
        <div className="overflow-x-auto px-3 pb-3">{adjustments.isLoading ? <div className="p-10"><Loader /></div> : <table className="w-full text-sm"><thead className="bg-gray-50"><tr><th className="border p-3"><input type="checkbox" checked={allSelected} onChange={() => setSelectedIds(allSelected ? [] : rows.map((entry) => entry.adjustmentId))} /></th>{exportColumns.concat('Action').map((heading) => <th key={heading} className="border p-3 text-left">{heading}</th>)}</tr></thead><tbody>{rows.length ? rows.map((entry: StockAdjustmentListItem) => <tr key={entry.adjustmentId} className="border-b even:bg-gray-50"><td className="border p-3"><input type="checkbox" checked={selectedIds.includes(entry.adjustmentId)} onChange={() => setSelectedIds((current) => current.includes(entry.adjustmentId) ? current.filter((id) => id !== entry.adjustmentId) : [...current, entry.adjustmentId])} /></td><td className="border p-3 font-semibold">{adjustmentCode(entry)}</td><td className="border p-3">{formatDate(entry.adjustmentDate)}</td><td className="border p-3">{entry.createdBy || user?.userName || 'admin'}</td><td className="border p-3">{formatDate(entry.createdAt || entry.adjustmentDate)}</td><td className="border p-3"><button onClick={() => navigate(`/stock/adjustments/${entry.adjustmentId}`)} className="text-blue-600"><Eye size={17} /></button></td></tr>) : <tr><td colSpan={6} className="bg-gray-50 p-5 text-center">No data available in table</td></tr>}</tbody></table>}</div>

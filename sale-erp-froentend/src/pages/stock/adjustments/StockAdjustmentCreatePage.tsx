...
export const StockAdjustmentCreatePage: React.FC = () => {
  ...
  const adjustment = useQuery({ queryKey: ['stock-adjustment', id], queryFn: () => stockAdjustmentApi.getById(id), enabled: isEdit });
  const warehouseRows = warehouses.data?.data || [];
  const mutation = useMutation({
    mutationFn: (payload: StockAdjustmentRequest) => isEdit ? stockAdjustmentApi.update(id, payload) : stockAdjustmentApi.create(payload),
    onSuccess: () => {
      toast.success(`Stock adjustment ${isEdit ? 'updated' : 'created'} successfully`);
      queryClient.invalidateQueries({ queryKey: ['stock-adjustments'] });
      queryClient.invalidateQueries({ queryKey: ['stock-adjustment', id] });
      navigate('/stock/adjustments');
    },
    onError: (error: any) => toast.error(error?.message || `Failed to ${isEdit ? 'update' : 'create'} stock adjustment`),
  });
  const totalQty = useMemo(() => lines.reduce((sum, line) => sum + Math.abs(line.actualQty - line.currentQty), 0), [lines]);

  useEffect(() => {
    if (!isEdit && !warehouseId && warehouseRows.length) setWarehouseId(warehouseRows[0].id);
  }, [isEdit, warehouseId, warehouseRows]);
  useEffect(() => {
    const detail = adjustment.data?.data;
    if (!detail) return;
    setAdjustmentDate(detail.adjustmentDate);
    setWarehouseId(detail.warehouse?.id || 0);
  ...
  if (isEdit && adjustment.isLoading) return <Loader />;

  return (
    <div className="space-y-5">
      <div className="text-sm text-gray-500">Home &gt; Stock &gt; Stock Adjustment List &gt; {isEdit ? 'Edit Adjustment' : 'New Adjustment'}</div>
      <div className="overflow-hidden rounded-lg bg-white shadow">
        ...
        <div className="flex gap-3 border-t p-5"><Button onClick={submit} isLoading={mutation.isPending}>{isEdit ? 'Update' : 'Submit'}</Button><Button variant="secondary" onClick={() => navigate('/stock/adjustments')}>Close</Button></div>
      </div>
    </div>
  );
};

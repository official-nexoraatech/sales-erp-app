...
export const StockTransferListPage: React.FC = () => {
  ...
  const remove = useMutation({
    mutationFn: stockTransferApi.delete,
    onSuccess: () => {
      toast.success('Stock transfer deleted successfully');
      queryClient.invalidateQueries({ queryKey: ['stock-transfers'] });
    },
    onError: (error: any) => toast.error(error?.message || 'Failed to delete stock transfer'),
  });
  ...
  const deleteSelected = async () => {
    if (!selectedIds.length) return toast.error('Select at least one stock transfer');
    const confirmed = await confirmAction({ title: 'Delete Stock Transfers', message: 'Delete selected stock transfers?', confirmText: 'Delete', variant: 'danger' });
    if (!confirmed) return;
    ...
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <h1 className="text-xl font-semibold uppercase text-gray-900">Stock Transfer List</h1>
          <Button onClick={() => navigate('/stock/transfers/create')} className="min-w-[150px]">New Transfer</Button>
        </div>
        ...
        <div className="overflow-x-auto px-3 pb-3">
          {transfers.isLoading ? <div className="p-10"><Loader /></div> : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50"><tr><th className="border p-3"><input type="checkbox" checked={allSelected} onChange={() => setSelectedIds(allSelected ? [] : rows.map((entry) => entry.transferId))} /></th>{exportColumns.concat('Action').map((heading) => <th key={heading} className="border p-3 text-left">{heading}</th>)}</tr></thead>
              <tbody>
                {rows.length ? rows.map((entry: StockTransferListItem) => (
                  <tr key={entry.transferId} className="border-b even:bg-gray-50">
                    <td className="border p-3"><input type="checkbox" checked={selectedIds.includes(entry.transferId)} onChange={() => setSelectedIds((current) => current.includes(entry.transferId) ? current.filter((id) => id !== entry.transferId) : [...current, entry.transferId])} /></td>
                    <td className="border p-3 font-semibold">{transferCode(entry)}</td>
                    <td className="border p-3">{formatDate(entry.transferDate)}</td>
                    <td className="border p-3">{entry.createdBy || user?.userName || 'admin'}</td>
                    <td className="border p-3">{formatDate(entry.createdAt || entry.transferDate)}</td>
                    <td className="border p-3"><div className="flex gap-2"><button title="View transfer" onClick={() => navigate(`/stock/transfers/${entry.transferId}`)} className="text-blue-600"><Eye size={17} /></button><button title="Edit transfer" onClick={() => navigate(`/stock/transfers/${entry.transferId}/edit`)} className="text-orange-600"><Edit size={17} /></button><button title="Delete transfer" onClick={async () => { if (await confirmAction({ title: 'Delete Stock Transfer', message: 'Delete this stock transfer?', confirmText: 'Delete', variant: 'danger' })) remove.mutate(entry.transferId); }} className="text-red-600"><Trash2 size={17} /></button></div></td>
                  </tr>

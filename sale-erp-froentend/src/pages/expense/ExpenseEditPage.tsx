...
export const ExpenseEditPage: React.FC = () => {
  ...
  const mutation = useMutation({ mutationFn: (payload: any) => expenseApi.update(id, payload), onSuccess: () => { toast.success('Expense updated successfully'); navigate('/expenses'); }, onError: (error: any) => toast.error(error?.message || 'Failed to update expense') });

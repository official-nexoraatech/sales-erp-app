...
export const PaymentInCreatePage: React.FC = () => {
  ...
  const paymentMethods = useQuery({ queryKey: ['payment-in-create-payment-methods'], queryFn: () => paymentMethodApi.getAll('') });
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => toast.error(error?.message || 'Failed to create payment in'),
  });

  const toggleSale = (id: number) => {
    setSaleIds((current) => current.includes(id) ? current.filter((entry) => entry !== id) : [...current, id]);
  };
  const submit = () => {
  ...
  return (
    <div className="space-y-5">
      <div className="text-sm text-gray-500">Home &gt; Sale &gt; Payment In &gt; Create Payment In</div>

...
import { paymentInApi } from '../../../api/endpoints';
...
export const PaymentInViewPage: React.FC = () => {
  const navigate = useNavigate();
  const id = Number(useParams<{ id: string }>().id);
  const { data, isLoading } = useQuery({ queryKey: ['payment-in', id], queryFn: () => paymentInApi.getById(id), enabled: id > 0 });
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={`Payment ${payment?.paymentNo || ''}`} actions={<Button variant="secondary" onClick={() => navigate('/sales/payment-in')}>Back</Button>} />
      <Card>
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div><p className="text-sm text-gray-500">Payment Type</p><p className="font-semibold">{payment?.paymentType || 'N/A'}</p></div>
          <div><p className="text-sm text-gray-500">Party</p><p className="font-semibold">{payment?.party?.name || 'N/A'}</p></div>

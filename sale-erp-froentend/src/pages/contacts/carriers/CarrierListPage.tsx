...
export const CarrierListPage: React.FC = () => {
  ...
  const columns = [
    ...
    { key: 'id', header: 'Action', render: (value: number) => <div className="flex gap-1"><button aria-label="View carrier" onClick={() => navigate(`/contacts/carriers/${value}`)} className="p-1 text-blue-600"><Eye size={17} /></button><button aria-label="Edit carrier" onClick={() => navigate(`/contacts/carriers/${value}/edit`)} className="p-1 text-orange-600"><Edit size={17} /></button><button aria-label="Delete carrier" onClick={() => setDeleteId(value)} className="p-1 text-red-600"><Trash2 size={17} /></button></div> },
  ];
  return <div className="space-y-6">
    <PageHeader title="Carrier List" actions={<Button onClick={() => navigate('/contacts/carriers/create')} className="flex items-center gap-2"><Plus size={18} />Create Carrier</Button>} />

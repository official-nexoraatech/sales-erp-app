...
export const CarrierListPage: React.FC = () => {
  ...
  const columns = [
    ...
    { key: 'id', header: 'Action', render: (value: number) => <div className="flex gap-1"><button title="View carrier" aria-label="View carrier" onClick={() => navigate(`/contacts/carriers/${value}`)} className="p-1 text-blue-600"><Eye size={17} /></button>{canUpdate && <button title="Edit carrier" aria-label="Edit carrier" onClick={() => navigate(`/contacts/carriers/${value}/edit`)} className="p-1 text-orange-600"><Edit size={17} /></button>}{canDelete && <button title="Delete carrier" aria-label="Delete carrier" onClick={() => setDeleteId(value)} className="p-1 text-red-600"><Trash2 size={17} /></button>}</div> },
  ];

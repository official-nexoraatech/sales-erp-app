...
// ─── Capability guard metrics (platform-sdk requireCapability) ─────────────
// Labelled by outcome (not just capability_key) so an operator can alert specifically on
// outcome="resolution_error" (capability-system infrastructure health) without that signal
// being drowned out by, or confused with, routine outcome="disabled" denial volume — a
// tenant simply not having a capability on its plan is expected, everyday traffic, while a
// resolution error means the capability system itself couldn't answer the question.

export const erpCapabilityCheckDeniedTotal = getOrCreateCounter({
...
// ─── PG-027 Session 2: tenant billing-cycle metrics ────────────────────────
// tenant_id deliberately NOT a label — same cardinality reasoning as the job-execution metrics
// above (tenant count grows unbounded; job_name-style per-tenant labelling isn't needed here
// since these are already scoped to one specific job).

export const erpTenantInvoiceGeneratedTotal = getOrCreateCounter({
  name: 'erp_tenant_invoice_generated_total',
  help: 'Total tenant billing invoices generated by the daily billing-cycle job',
});

export const erpTenantPaymentFailedTotal = getOrCreateCounter({
  name: 'erp_tenant_payment_failed_total',
  help: 'Total tenant billing payment failures',
  labelNames: ['reason'],
});

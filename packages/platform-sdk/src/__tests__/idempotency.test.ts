...
describe('isUniqueConstraintViolation', () => {
  it('returns true for a matching Postgres 23505 error', () => {
    expect(isUniqueConstraintViolation(uniqueViolation('invoices_tenant_client_operation_id'), 'invoices_tenant_client_operation_id')).toBe(true);
  });

  it('returns false when the constraint name differs', () => {
    expect(isUniqueConstraintViolation(uniqueViolation('invoices_tenant_number'), 'invoices_tenant_client_operation_id')).toBe(false);
  });

  it('returns false for a non-Postgres error', () => {
    expect(isUniqueConstraintViolation(new Error('boom'), 'invoices_tenant_client_operation_id')).toBe(false);
...
describe('withIdempotentInsert', () => {
  ...
  it('resolves normally when fn succeeds', async () => {
    const result = await withIdempotentInsert(() => Promise.resolve(42), 'invoices_tenant_client_operation_id', 'op-123');
...
describe('deriveTimeBucketedDedupKey', () => {
  it('produces a stable hash within the same time bucket', () => {
    const now = Date.now();
    const a = deriveTimeBucketedDedupKey(1, 'INVOICE_REMINDER', 'SMS', '+911234567890', { invoiceId: 1 }, 5 * 60 * 1000);
    const b = deriveTimeBucketedDedupKey(1, 'INVOICE_REMINDER', 'SMS', '+911234567890', { invoiceId: 1 }, 5 * 60 * 1000);
    expect(a).toBe(b);
  ...
  it('produces a different hash across buckets', () => {
    ...
    const a = deriveTimeBucketedDedupKey(1, 'INVOICE_REMINDER', 'SMS', '+911234567890', { invoiceId: 1 }, bucketMs);
    const b = deriveTimeBucketedDedupKey(1, 'INVOICE_REMINDER', 'SMS', '+911234567890', { invoiceId: 1 }, 1);
    expect(a).not.toBe(b);
  });
  ...
  it('produces a different hash for a different tenant/recipient', () => {
    const bucketMs = 5 * 60 * 1000;
    const a = deriveTimeBucketedDedupKey(1, 'INVOICE_REMINDER', 'SMS', '+911234567890', { invoiceId: 1 }, bucketMs);
    const b = deriveTimeBucketedDedupKey(2, 'INVOICE_REMINDER', 'SMS', '+911234567890', { invoiceId: 1 }, bucketMs);
    expect(a).not.toBe(b);
  });
});

...
import { WorkflowApprovalRequiredError, NotFoundError, BusinessError } from '@erp/types';
...
export const SYSTEM_WORKFLOW_DEFINITIONS: Array<{
  ...
  triggerEvent: string;
  entityType: string;
  conditionExpr: { field: string; operator: 'GT' | 'LT' | 'GTE' | 'LTE' | 'EQ' | 'ALWAYS'; value?: number };
  nodes: WorkflowNode[];
  timeoutHours: number;
  isSystem: boolean;
}> = [
  {
    ...
    nodes: [{ id: 'node_1', name: 'Sales Manager Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'SALES_MANAGER' }],
    timeoutHours: 24, isSystem: true,
  },
  {
  ...
  },
  {
    ...
    nodes: [
      { id: 'node_1', name: 'Purchase Manager Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'PURCHASE_MANAGER', nextNodeId: 'node_2' },
      { id: 'node_2', name: 'Owner Final Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'OWNER' },
    ],
    timeoutHours: 48, isSystem: true,
  },
  {
    ...
    nodes: [{ id: 'node_1', name: 'Purchase Manager Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'PURCHASE_MANAGER' }],
    timeoutHours: 12, isSystem: true,
  },
  {
    ...
    nodes: [{ id: 'node_1', name: 'Department Head Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'ADMIN' }],
    timeoutHours: 24, isSystem: true,
  },
  {
    ...
    nodes: [{ id: 'node_1', name: 'Inventory Manager Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'INVENTORY_MANAGER' }],
    timeoutHours: 24, isSystem: true,
  },
  {
    ...
    nodes: [{ id: 'node_1', name: 'Owner Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'OWNER' }],
    timeoutHours: 48, isSystem: true,
  },
  {
    ...
    conditionExpr: { field: '', operator: 'ALWAYS' },
    nodes: [{ id: 'node_1', name: 'Owner Final Confirmation', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'OWNER' }],
    timeoutHours: 72, isSystem: true,
  },
  {
  ...
  },
  {
    ...
    nodes: [
      { id: 'node_1', name: 'Purchase Manager Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'PURCHASE_MANAGER', nextNodeId: 'node_2' },
      { id: 'node_2', name: 'Owner Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'OWNER' },
    ],
    timeoutHours: 24, isSystem: true,
  },
  {
    ...
    entityType: 'Employee',
    conditionExpr: { field: '', operator: 'ALWAYS' },
    nodes: [{ id: 'node_1', name: 'HR Manager Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'HR_MANAGER' }],
    timeoutHours: 48, isSystem: true,
  },
  {
    name: 'Leave Approval',
    ...
    entityType: 'Leave',
    conditionExpr: { field: '', operator: 'ALWAYS' },
    nodes: [{ id: 'node_1', name: 'Manager Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'HR_MANAGER' }],
    timeoutHours: 24, isSystem: true,
  },
  {
  ...
  },
  {
    ...
    nodes: [{ id: 'node_1', name: 'Admin Approval', type: 'APPROVAL', approverType: 'ROLE', approverRef: 'ADMIN' }],
    timeoutHours: 24, isSystem: true,
  },
  {
...
export class WorkflowEngine {
  ...
  async trigger(input: WorkflowTriggerInput): Promise<typeof workflowInstances.$inferSelect | null> {
    ...
    const condition = definition.conditionExpr as { field: string; operator: string; value?: number };
    ...
    // Resolve approver and create approval record for first node
    const approverId = await this.resolveApprover(firstNode);
    await this.createApprovalRecord(instance.id, firstNode, approverId, input.userId);
  ...
  async getStatus(instanceId: number): Promise<WorkflowStatus> {
    const [instance] = await this.db
      ...
      .where(and(eq(workflowInstances.id, instanceId), eq(workflowInstances.tenantId, this.tenantId)));
    ...
    const approvals = await this.db
      ...
      .where(and(eq(workflowApprovals.instanceId, instanceId), eq(workflowApprovals.tenantId, this.tenantId)));
  ...
  private async processDecision(
    ...
    if (currentNode.nextNodeId) {
      ...
      if (nextNode) {
        ...
        const approverId = await this.resolveApprover(nextNode);
        await this.createApprovalRecord(instance.id, nextNode, approverId, input.userId);
        return;
      }
    }
  ...
  private async resolveApprover(node: WorkflowNode): Promise<number> {
    if (node.approverType === 'ROLE') {
      // Find first user with this role — simplified for Phase 1
      // Phase 2+ will do proper role→user resolution with branch scoping
    ...
    return parseInt(node.approverRef, 10) || 0;
  }

...
const ScheduleCreateSchema = z.object({
  ...
  params: z
    ...
    .optional()
    .default({}),
  // PDF defaults removed: ScheduledReportJob.runSchedule() only builds an email attachment for
  // EXCEL/CSV — a PDF-format schedule (or any caller that omits `format`) previously fell back
  // to PDF and silently emailed recipients with no attachment at all. SchedulesPage.tsx already
  // never sends PDF for the same reason; this closes the same gap for any other caller.
  format: z.enum(['PDF', 'EXCEL', 'CSV']).optional().default('EXCEL'),
...
export async function analyticsReportsRoutes(
    ...
    async (req, reply) => {
      ...
      if (!req.auth.permissions.includes(definition.permission)) {
        return reply.code(403).send({
          error: { code: 'FORBIDDEN', message: `Missing permission: ${definition.permission}` },
        });
      }
      ...
      if (format === 'CSV') {
        ...
        return reply
          ...
          .header(
            'Content-Disposition',
            `attachment; filename="${formatter.getFileName(req.params.slug, 'CSV')}"`
          )
          .send(csv);
      }
      ...
      if (format === 'EXCEL') {
        ...
        return reply
          ...
          .header(
            'Content-Disposition',
            `attachment; filename="${formatter.getFileName(req.params.slug, 'EXCEL')}"`
          )
          .send(buf);
      }
  ...
  fastify.get<{ Params: { runId: string } }>(
    '/api/v2/reports/run-history/:runId',
    {
      preHandler: [authenticate, requirePermission('REPORT_VIEW')],
    },
    async (req, reply) => {
  ...
  fastify.delete<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      if (!deleted) {
        return reply
          .code(404)
          .send({ error: { code: 'NOT_FOUND', message: 'Schedule not found' } });
      }
  ...
  fastify.get(
    ...
    async (req, reply) => {
      ...
      return reply.code(200).send({
        data: result.rows,
        meta: { total: result.totalRows, generatedAt: result.generatedAt },
      });
    }
  );

...
import type { PdfEngine } from '../domain/PdfEngine.js';
...
export async function reportRoutes(
  ...
  fastify.post(
    '/internal/reports/outstanding-summary',
    ...
    async (request, reply) => {
      ...
      return reply.code(200).send({
        data: {
          arTotal,
          apTotal,
          customersWithOutstanding: ar.rows.length,
          suppliersWithOutstanding: ap.rows.length,
        },
      });
    }
  );
  ...
  fastify.post('/reports/pdf', { preHandler: checkInternalKey }, async (request, reply) => {
    ...
    return reply
      ...
      .header(
        'Content-Disposition',
        `attachment; filename="${body.data.documentType.toLowerCase()}.pdf"`
      )
      .send(pdf);
  });
  ...
  fastify.post<{ Params: { type: string } }>(
    '/config/number-series/:type',
    { preHandler: [authenticate, requirePermission(PERMISSIONS.NUMBER_SERIES_CONFIG)] },
    async (request, reply) => {
  ...
  fastify.post<{ Params: { type: string } }>(
    '/config/number-series/:type/preview',
    { preHandler: [authenticate, requirePermission(PERMISSIONS.NUMBER_SERIES_CONFIG)] },
    async (request, reply) => {
      const { tenantId } = (request as unknown as AuthedRequest).auth;
      const type = request.params.type.toUpperCase() as SeriesType;
  ...
  fastify.post<{ Params: { type: string } }>(
    '/internal/number-series/:type/next',

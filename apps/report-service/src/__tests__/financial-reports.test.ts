...
describe('ES-17 — Financial statement correctness', () => {
  ...
  it('profit-loss-report: REVENUE minus EXPENSE equals NET PROFIT', async () => {
    ...
    const result = await engine.generate('profit-loss-report', TENANT_A, {
      fromDate: '2026-04-01',
      toDate: '2026-06-30',
    });
  ...
  it('balance-sheet-report: total ASSETS equals total LIABILITY + EQUITY', async () => {
    ...
    const result = await engine.generate('balance-sheet-report', TENANT_A, {
      asOfDate: '2026-06-30',
    });

    const totalAssets = result.rows
      .filter((r) => r['section'] === 'ASSET')
      .reduce((s, r) => s + Number(r['amount']), 0);
  ...
  it('trial-balance-report: total closing DEBIT equals total closing CREDIT', async () => {
    ...
    const result = await engine.generate('trial-balance-report', TENANT_A, {
      fromDate: '2026-04-01',
      toDate: '2026-06-30',
    });
  ...
  it('profit-loss-report: tenant isolation — query values contain TENANT_A, never TENANT_B', async () => {
    ...
    await engine.generate('profit-loss-report', TENANT_A, {
      fromDate: '2026-04-01',
      toDate: '2026-06-30',
    });
...
describe('ES-17 — cash-flow-report', () => {
  ...
  it('computes closing cash as opening cash plus net movement', async () => {
    // Audit 2026-07-23: cash-flow-report now classifies each cash movement by its counter-
    // account's sub-type (3-way Operating/Investing/Financing, matching accounting-service's
    // ReportsEngine.getCashFlow()) instead of bucketing everything as "Operating" — so the
    // query shape changed from a single aggregate {total_in, total_out} row to one row per
    // movement with {net_amount, counter_sub_type}. Neither counter type here is a Fixed Asset/
    // Long-term Liability/Equity account, so both fall through to Operating, same net result.
    const db = makeDb([
      [
        { net_amount: '80000', counter_sub_type: 'ACCOUNTS_RECEIVABLE' },
        { net_amount: '-30000', counter_sub_type: 'ACCOUNTS_PAYABLE' },
      ],
      [{ balance: '20000' }],
    ]);
    const engine = new ReportEngine(db as never);

    const result = await engine.generate('cash-flow-report', TENANT_A, {
      fromDate: '2026-04-01',
      toDate: '2026-06-30',
    });

    const summary = Object.fromEntries(
      result.rows.map((r) => [r['description'], Number(r['amount'])])
    );
  ...
  it('classifies a fixed-asset counter-account as Investing, not Operating', async () => {
    const db = makeDb([
      [{ net_amount: '-500000', counter_sub_type: 'FIXED_ASSET' }],
      [{ balance: '0' }],
    ]);
    const engine = new ReportEngine(db as never);
    ...
    expect(summary['Purchase of fixed assets']).toBe(-500000);
    expect(summary['Net Cash from Operating Activities']).toBe(0);
    expect(summary['Net Cash from Investing Activities']).toBe(-500000);
  });
});
...
describe('ES-26 (M5) — profit-loss-report CONTRA account categorization', () => {
  it('categorizes a CONTRA account as CONTRA_REVENUE (not OTHER) and nets it into COGS matching accounting-service math', async () => {
    ...
    const sum = (cat: string) =>
      result.rows.filter((r) => r['category'] === cat).reduce((s, r) => s + Number(r['amount']), 0);
...
describe('ES-26 (M6) — broken report columns fixed', () => {
  ...
  const cases: Array<{ slug: string; params: Record<string, string | number> }> = [
    ...
    {
      slug: 'account-ledger',
      params: { fromDate: '2026-04-01', toDate: '2026-06-30', accountId: 1 },
    },
    { slug: 'expense-analysis', params: { fromDate: '2026-04-01', toDate: '2026-06-30' } },
    {
      slug: 'bank-book',
      params: { fromDate: '2026-04-01', toDate: '2026-06-30', bankAccountId: 1 },
    },
    { slug: 'fund-flow', params: { fromDate: '2026-04-01', toDate: '2026-06-30' } },
  ];

  it.each(cases)(
    '$slug references real financial_entries columns, never the nonexistent ones',
    async ({ slug, params }) => {
      const db = makeDb([[]]);
      const engine = new ReportEngine(db as never);
...
describe('ES-26 (M7) — gst-payable-report cache', () => {
  ...
  it('a second call within the TTL is served from cache and does not re-query Postgres', async () => {
    const rows = [
      { gst_type: 'CGST', output_tax: '1000', itc_available: '200', net_payable: '800' },
    ];
  ...
  it('a different date range is cached independently and does re-query Postgres', async () => {
    ...
    await engine.generate('gst-payable-report', TENANT_A, {
      fromDate: '2026-04-01',
      toDate: '2026-06-30',
    });
    await engine.generate('gst-payable-report', TENANT_A, {

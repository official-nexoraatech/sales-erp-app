...
// customer-role-rejected-from-staff-routes.test.ts exactly for the PARTNER auth scope. The
// self-service notification inbox (GET /notifications and friends) scopes every route to the
// caller's own userId — a PARTNER-role JWT's numeric id could coincidentally collide with a
// real employee's users.id and reach that employee's own in-app notification inbox.
...
describe('GET /notifications rejects a PARTNER-role token', () => {

...
export async function syncRoutes(
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const conditions = [eq(items.tenantId, ctx.tenant.tenantId), isNull(items.deletedAt)];
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const conditions = [eq(priceListItems.tenantId, ctx.tenant.tenantId)];
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const byHsn = new Map<
        string,
        { hsnCode: string; gstRate: number; cessRate: number; updatedAt: string }
      >();
      for (const r of rows) {
      ...
      const content = deduped
        ...
        .map((r) => ({ ...r, tenantId: ctx.tenant.tenantId }));

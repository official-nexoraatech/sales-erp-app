...
// Phase 9 GUC-per-request rollout — migrated 2026-08-21. No external I/O — FabricRollService has
// no fetch() calls.
export async function fabricRollRoutes(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      if (!itemId) {
        const rows = await ctx.db.raw
          .select()
          .from(fabricRolls)
          .where(eq(fabricRolls.tenantId, ctx.tenant.tenantId))
          .orderBy(desc(fabricRolls.receivedAt));
        return reply.code(200).send({ data: rows });
      }

      const svc = new FabricRollService(ctx.db.raw);
      const rolls = await svc.getAvailableRolls(parseInt(itemId, 10), ctx.tenant.tenantId);
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const { id } = request.params as { id: string };
      const svc = new FabricRollService(ctx.db.raw);
      const result = await svc.getCutHistory(parseInt(id, 10), ctx.tenant.tenantId);

...
export async function categoryRoutes(
  ...
  fastify.get(
    '/categories',
    { preHandler: [authenticate, requirePermission(PERMISSIONS.CATEGORY_VIEW)] },
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const rows = await ctx.db.raw
        .select()
        .from(categories)
        .where(and(eq(categories.tenantId, ctx.tenant.tenantId), isNull(categories.deletedAt)));
  ...
  fastify.post(
    '/categories',
    { preHandler: [authenticate, requirePermission(PERMISSIONS.CATEGORY_CREATE)] },
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
  ...
  fastify.put(
    '/categories/:id',
    { preHandler: [authenticate, requirePermission(PERMISSIONS.CATEGORY_UPDATE)] },
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const { id: idParam } = request.params as { id: string };
      const id = parseInt(idParam, 10);
  ...
  fastify.delete(
    '/categories/:id',
    { preHandler: [authenticate, requirePermission(PERMISSIONS.CATEGORY_DELETE)] },
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const { id: idParam } = request.params as { id: string };
      const id = parseInt(idParam, 10);
      const [existing] = await ctx.db.raw

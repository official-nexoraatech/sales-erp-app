...
// POST /inventory/reservations/expire is deliberately NOT migrated:
// ReservationEngine.expireStale() has no tenant filter at all — it's a cross-tenant scheduler
// batch job (checklist caveat 4e), the same shape as sales-service's expire-stale jobs.
export async function reservationRoutes(
  ...
  fastify.post('/inventory/reservations/expire', async (request, reply) => {
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const body = CreateReservationSchema.parse(
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const active = await ctx.db.raw
        ...
        .where(
          and(
            eq(stockReservations.tenantId, ctx.tenant.tenantId),
  ...
  fastify.delete(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const body = ReleaseSchema.parse(
        (request.body as { data?: unknown })?.data ?? request.body ?? {}
      );
      const engine = new ReservationEngine(ctx.db.raw);

      await engine.release(parseInt(id, 10), ctx.tenant.tenantId, body.reason);

...
// caveat 5b) — each route is single-tenant (tenantId from body/query), so each uses
// withTenantConnection(ctxFactory.rawDb, tenantId, ...) directly. The reminder route's
// notification fetch() runs strictly after its DB read completes, so only the read is wrapped.
export async function internalRoutes(
  ...
  fastify.post('/internal/ledger', {
    ...
    handler: async (req, reply) => {
      ...
      await withTenantConnection(ctxFactory.rawDb, body.tenantId, async (db) => {
  ...
  fastify.post(
    '/internal/inventory/valuation-snapshot',
    { preHandler: checkInternalKey },
    async (request, reply) => {
      ...
      const result = await withTenantConnection(ctxFactory.rawDb, tenantId, async (db) => {
        ...
        const totalStockValue = rows.reduce(
          (sum, r) => sum + parseFloat(String(r.currentStockValue ?? 0)),
          0
        );
        ...
        return { asOfDate, totalStockValue, itemCount: rows.length };
      });
  ...
  fastify.post(
    '/internal/inventory/physical-verification-reminder',
    { preHandler: checkInternalKey },
    async (request, reply) => {
      const tenantId = parseInt((request.query as { tenantId?: string }).tenantId ?? '', 10);
      if (!tenantId)

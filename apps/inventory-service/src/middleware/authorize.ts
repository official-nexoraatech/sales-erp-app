...
// currently gates on a broader/adjacent permission (e.g. WAREHOUSE_MANAGE) for historical
// reasons but a purpose-built constant also exists (e.g. STOCK_ADJUST) — checking both keeps
// every role's current access working while making the purpose-built constant actually do
// something, instead of silently replacing the check and risking revoking access from a role
// that only has the broader one.

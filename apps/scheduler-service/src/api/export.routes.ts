...
export async function exportRoutes(
  ...
  fastify.post('/exports/generate', { preHandler: authenticate }, async (request, reply) => {
    ...
    if (body.format === 'PDF') {
      return reply.code(400).send({
        error: {
          ...
          message:
            'PDF export is only available for individual documents (see report-service /reports/:id/pdf); use CSV or XLSX for bulk entity export',
        },
      });
    }
    ...
    return reply
      .code(201)
      .send({ data: { jobId, fileName, downloadUrl, expiresAt: signedUrlExpiresAt } });
  });
  ...
  fastify.get<{ Params: { jobId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (job.status !== 'READY') {
        return reply
          .code(202)
          .send({ data: { status: job.status, message: 'Export not ready yet' } });
      }

      if (job.signedUrlExpiresAt && new Date() > job.signedUrlExpiresAt) {
        return reply
          .code(410)
          .send({ error: { code: 'EXPORT_EXPIRED', message: 'Export link has expired' } });
      }
  ...
  fastify.get<{ Params: { jobId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply
        .code(200)
        .send({ data: { status: job.status, expiresAt: job.signedUrlExpiresAt } });
    }
  );
}

...
async function callInventoryService(inventoryUrl: string, path: string, apiKey: string): Promise<Response> {
...
export function registerSystemJobs(registry: JobRegistry, db: ErpDatabase, storage: StorageClient): void {
  // ── ACCOUNTING ─────────────────────────────────────────────────────────────
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      try {
        const res = await fetch(`${accountingUrl}/api/v2/internal/reports/trial-balance-snapshot?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: { isBalanced?: boolean; totalDebit?: number; totalCredit?: number } };
        logger.info({ tenantId, isBalanced: body.data?.isBalanced, totalDebit: body.data?.totalDebit, totalCredit: body.data?.totalCredit }, 'Trial balance snapshot complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${reportUrl}/internal/reports/outstanding-summary?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: { arTotal?: number; apTotal?: number } };
        logger.info({ tenantId, arTotal: body.data?.arTotal, apTotal: body.data?.apTotal }, 'Outstanding report complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${accountingUrl}/api/v2/internal/bank-reconciliation/reminder?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: { unreconciledAccounts?: number; totalUnmatched?: number } };
        logger.info({ tenantId, unreconciledAccounts: body.data?.unreconciledAccounts, totalUnmatched: body.data?.totalUnmatched }, 'Bank reconciliation reminder complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${productionUrl}/api/v2/internal/inventory/reorder-required?tenantId=${tenantId}`, {
          method: 'GET',
  ...
  registry.register(
    ...
    async (_job) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await inventoryServiceBreaker.fire(inventoryUrl, '/api/v2/inventory/reservations/expire', apiKey);
  ...
  registry.register(
    ...
    async (_job) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await inventoryServiceBreaker.fire(inventoryUrl, '/api/v2/inventory/reconcile', apiKey);
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${inventoryUrl}/api/v2/internal/inventory/valuation-snapshot?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: { totalStockValue?: number; itemCount?: number } };
        logger.info({ tenantId, totalStockValue: body.data?.totalStockValue, itemCount: body.data?.itemCount }, 'Stock value report complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${inventoryUrl}/api/v2/internal/inventory/physical-verification-reminder?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      try {
        ...
        const body = await res.json() as { data?: { period?: string; isExportReady?: boolean; validationErrorCount?: number } };
        logger.info({ tenantId, period: body.data?.period, isExportReady: body.data?.isExportReady, validationErrorCount: body.data?.validationErrorCount }, 'GSTR-1 auto-prepare complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      try {
        ...
        logger.info({ tenantId, period: body.data?.period, sent: body.data?.sent }, 'GSTR-3B reminder complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      try {
        ...
        const body = await res.json() as { data?: { period?: string; summary?: { matched?: number; booksOnly?: number; gstr2aOnly?: number; amountMismatch?: number } } };
        logger.info({ tenantId, period: body.data?.period, summary: body.data?.summary }, 'GSTR-2A reconciliation complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      try {
        ...
        const body = await res.json() as { data?: { payrollRunId?: number; employeeCount?: number; totalNet?: number } };
        logger.info({ tenantId, payrollRunId: body.data?.payrollRunId, employeeCount: body.data?.employeeCount, totalNet: body.data?.totalNet }, 'Payroll prepare complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      try {
        ...
        logger.info({ tenantId, payrollRunId: body.data?.payrollRunId, count: body.data?.count }, 'Salary slip email dispatch complete');
      } catch (err) {
  ...
  registry.register(
    ...
    { cron: '0 10 * * 1,3,5', description: 'Send payment reminders for overdue invoices MWF — ES-18', tenantScoped: false },
    async (_job) => {
      const salesUrl = process.env['SALES_SERVICE_URL'] ?? 'http://localhost:3013';
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        ...
        logger.info({ candidates: body.data?.candidates, reminded: body.data?.reminded }, 'Overdue payment reminder dispatch complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${salesUrl}/api/v2/crm/credit-limit-review/run?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: { atRiskCount?: number } };
        logger.info({ tenantId, atRiskCount: body.data?.atRiskCount }, 'Credit limit review complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${purchaseUrl}/api/v2/purchase/po-delivery-reminders/send?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: { pendingCount?: number; reminded?: number } };
        logger.info({ tenantId, pendingCount: body.data?.pendingCount, reminded: body.data?.reminded }, 'PO delivery reminder complete');
      } catch (err) {
  ...
  registry.register(
    ...
    async (_job, tenantId) => {
      ...
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${purchaseUrl}/api/v2/purchase/pending-grn-alerts/run?tenantId=${tenantId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: { pendingCount?: number } };
        logger.info({ tenantId, pendingCount: body.data?.pendingCount }, 'Pending GRN alert complete');
      } catch (err) {
    ...
    async (_job, tenantId) => {
      ...
      try {
        const overdue = await db
          ...
          .where(and(eq(workflowInstances.tenantId, tenantId), eq(workflowInstances.status, 'PENDING'), lt(workflowInstances.expiresAt, new Date())));

        let escalated = 0;
        let expired = 0;
        for (const instance of overdue) {
          const [definition] = await db.select().from(workflowDefinitions).where(eq(workflowDefinitions.id, instance.definitionId));
          if (definition?.escalationUserId) {
            await db
              .update(workflowApprovals)
              .set({ action: 'ESCALATED', decidedAt: new Date(), updatedAt: new Date() })
              .where(and(eq(workflowApprovals.instanceId, instance.id), eq(workflowApprovals.nodeId, instance.currentNodeId ?? ''), eq(workflowApprovals.action, 'PENDING')));
            ...
            await db.update(workflowInstances).set({ updatedAt: new Date() }).where(eq(workflowInstances.id, instance.id));
            escalated++;
          } else {
            // No escalation target configured — nothing left to do but mark it expired.
            await db.update(workflowInstances).set({ status: 'EXPIRED', completedAt: new Date(), updatedAt: new Date() }).where(eq(workflowInstances.id, instance.id));
            expired++;
          }
        }
    ...
    async (_job, tenantId) => {
      ...
      try {
        const pending = await db
          ...
          .where(and(eq(workflowApprovals.tenantId, tenantId), eq(workflowApprovals.action, 'PENDING')));

        for (const approval of pending) {
          await db
            .update(workflowApprovals)
            .set({ reminderCount: sql`${workflowApprovals.reminderCount} + 1`, notifiedAt: new Date(), updatedAt: new Date() })
            .where(eq(workflowApprovals.id, approval.id));
        }

        logger.info({ tenantId, remindedCount: pending.length }, 'Workflow approval reminder bookkeeping complete');
      } catch (err) {
        ...
        const deleted = await deleteBatched(async () => {
          const rows = await db.execute(sql`
            ...
            WHERE id IN (
              ...
              WHERE ${outboxEvents.published} = true AND ${outboxEvents.createdAt} < ${cutoff}
  ...
  registry.register(
    ...
    async () => {
      try {
        ...
        let totalArchived = 0;
        for (let i = 0; i < MAX_BATCHES_PER_RUN; i++) {
          const rows = await db.select().from(auditLog).where(lt(auditLog.createdAt, cutoff)).limit(DELETE_BATCH_SIZE);
          ...
          await db.delete(auditLog).where(and(lt(auditLog.createdAt, cutoff), sql`${auditLog.id} = ANY(${rows.map((r) => r.id)})`));
      ...
      try {
        ...
        const refreshDeleted = await deleteBatched(async () => {
          const rows = await db.execute(sql`
            ...
            WHERE id IN (SELECT id FROM ${refreshTokens} WHERE ${refreshTokens.expiresAt} < ${now} LIMIT ${DELETE_BATCH_SIZE})
            RETURNING id
          `);
          return (rows as unknown[]).length;
        });
        const resetDeleted = await deleteBatched(async () => {
          const rows = await db.execute(sql`
            ...
            WHERE id IN (SELECT id FROM ${passwordResetTokens} WHERE ${passwordResetTokens.expiresAt} < ${now} LIMIT ${DELETE_BATCH_SIZE})
            RETURNING id
          `);
          return (rows as unknown[]).length;
        });
  ...
  registry.register(
    ...
    async () => {
      try {
        ...
        const rows = await db
          .select({ id: importJobs.id, s3Key: importJobs.s3Key, errorReportS3Key: importJobs.errorReportS3Key })
          .from(importJobs)
          .where(and(sql`${importJobs.status} IN ('COMPLETED', 'FAILED', 'ROLLED_BACK')`, lt(importJobs.createdAt, cutoff)))
          .limit(DELETE_BATCH_SIZE);
  ...
  registry.register(
    ...
    async () => {
      try {
        ...
        let totalArchived = 0;
        for (let i = 0; i < MAX_BATCHES_PER_RUN; i++) {
          const rows = await db.select().from(notificationLog).where(lt(notificationLog.createdAt, cutoff)).limit(DELETE_BATCH_SIZE);
          if (rows.length === 0) break;
          ...
          await db.delete(notificationLog).where(sql`${notificationLog.id} = ANY(${rows.map((r) => r.id)})`);
          totalArchived += rows.length;
  ...
  registry.register(
    ...
    async () => {
      try {
        ...
        if (rows.length > 0) {
          await db.update(exportJobs).set({ status: 'EXPIRED', s3Key: null, signedUrl: null, updatedAt: new Date() }).where(sql`${exportJobs.id} = ANY(${rows.map((r) => r.id)})`);
        }
  ...
  registry.register(
    ...
    { cron: '0 9 1 1,4,7,10 *', description: 'Quarterly reminder to re-run the DR drill (infrastructure/runbooks/dr-runbook.md)', tenantScoped: false },
    async () => {
  ...
  registry.register(
    ...
    { cron: '0 9 * * *', description: 'Daily 09:00 — check reorder levels and email report to purchase manager', tenantScoped: true },
    async (_job, tenantId) => {
      if (tenantId === undefined) return;
      const productionUrl = process.env['PRODUCTION_SERVICE_URL'] ?? 'http://localhost:3022';
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
  ...
  registry.register(
    ...
    { cron: '0 9 * * *', description: 'Daily 09:00 — alert for overdue job work orders', tenantScoped: true },
    async (_job, tenantId) => {
      if (tenantId === undefined) return;
      const productionUrl = process.env['PRODUCTION_SERVICE_URL'] ?? 'http://localhost:3022';
      const apiKey = process.env['INTERNAL_API_KEY'] ?? '';
      try {
        const res = await fetch(`${productionUrl}/api/v2/internal/job-work-orders/in-progress?tenantId=${tenantId}`, {
          method: 'GET',
          headers: { 'x-internal-key': apiKey },
        });
        const body = await res.json() as { data?: unknown[] };
        logger.info({ tenantId, inProgressCount: body.data?.length ?? 0 }, 'Job work overdue alert check complete');
      } catch (err) {

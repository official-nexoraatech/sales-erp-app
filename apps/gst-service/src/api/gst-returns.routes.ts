...
// GstReturnTrackerService nor Gstr3bService has fetch() calls.
export async function gstReturnsRoutes(
  ...
  fastify.post(
    '/gst/returns/:returnType/mark-filed',
    {
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const returnType = (request.params as { returnType: string })
        .returnType as (typeof RETURN_TYPES)[number];
      if (!RETURN_TYPES.includes(returnType)) {
        throw new ValidationError(
          `Invalid return type. Must be one of: ${RETURN_TYPES.join(', ')}`
        );
      }
      ...
      if (returnType === 'GSTR3B') {
        ...
        const gstr3bResult = await Gstr3bService.compute(
          ctx.db,
          tenantId,
          body.data.period,
      ...
      await GstReturnTrackerService.markFiled(
        ctx.db,
        tenantId,
        userId,
        returnType,
      ...
      return reply
        .code(200)
        .send({ data: { message: `${returnType} for ${body.data.period} marked as filed` } });
    })
  );

...
// Gstr1Service nor Gstr1ExcelFormatter has fetch() calls.
export async function gstr1Routes(
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const BodySchema = z
        .object({ gstin: z.string().length(15, 'GSTIN must be 15 characters') })
        .optional();
      ...
      if (validationErrors.length > 0) {
        throw new BusinessError(
          'GSTR1_VALIDATION_FAILED',
          'GSTR-1 validation failed before export',
      ...
      return reply
        ...
        .header(
          'Content-Disposition',
          `attachment; filename="${Gstr1ExcelFormatter.getFileName(q.data.period)}"`
        )
        .send(buf);
    })
  );
    ...
    handler: async (request, reply) => {
      ...
      const result = await withTenantConnection(ctxFactory.rawDb, tenantId, async (db) => {
        ...
        await ctx.audit.log({
          ...
          entityType: 'GSTR1',
          entityId: tenantId,
          after: { period, isExportReady, validationErrorCount: validationErrors.length } as Record<
            string,
            unknown
          >,
        });

        return { isExportReady, validationErrorCount: validationErrors.length };
      });

      return reply.code(200).send({ data: { period, ...result } });
    },
  });
}

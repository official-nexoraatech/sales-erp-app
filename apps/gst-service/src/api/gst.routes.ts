...
// Phase 9 GUC-per-request rollout — migrated 2026-08-21. No external I/O anywhere in this
// file. POST /gst/compute is pure computation (GSTCalculator) with no DB access at all, so
// there's no GUC gap to close there — left unchanged.
export async function gstRoutes(
  ...
  fastify.put(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const [existing] = await ctx.db.raw
        .select()
        .from(gstRates)
        .where(and(eq(gstRates.id, id), eq(gstRates.tenantId, tenantId)));
  ...
  fastify.post(
    '/gst/compute',
    { preHandler: [authenticate, requirePermission(PERMISSIONS.GST_COMPUTE)] },
    async (request, reply) => {
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      await ctx.db.raw
        .insert(gstRates)
        .values(defaultRates.map((r) => ({ tenantId, createdBy: userId, ...r })))
        .onConflictDoNothing();

      return reply
        .code(200)
        .send({ data: { message: 'GST rates seeded', count: defaultRates.length } });
    })
  );
}

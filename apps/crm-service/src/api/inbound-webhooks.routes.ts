...
// Phase 9 GUC-per-request rollout — deliberately NOT migrated, same reasoning as
// referral-public.routes.ts. The tenant isn't known until an internal lookup resolves it
// (resolveTenantByAddress for WhatsApp/Instagram/Email/SMS, a CallSid lookup for the Twilio
// callbacks), and several of these routes loop over a batch payload that can in principle
// span more than one resolved tenant within a single request — there's no single tenantId to
// scope the whole handler with up front the way tenantScopedHandler/withTenantConnection expect.
// CallService.handleStatusCallback()/handleRecordingCallback() also do their own multi-step
// lookup+write internally on a plain db handle, same as ReferralService.redeem() — closing the
// gap here would mean restructuring those service methods to separate "resolve tenant" from
// "do the scoped work," a service-level design change, not a route-file swap. Left as explicit
// future work alongside caveat 4e's class of deferred cases.

 ...
 *
 * Phase 9 GUC-per-request rollout — migrated 2026-08-21. No PlatformContextFactory (API-key
 * auth, not req.auth) — the tenant is known once requirePublicApiScope's preHandler resolves
 * request.apiKeyAuth, so each route wraps its DB read in withTenantConnection(db, tenantId, ...).
 */

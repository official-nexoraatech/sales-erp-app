...
//
// Phase 9 GUC-per-request rollout — migrated 2026-08-21. No external I/O — AccountService has no
// fetch() calls. ctx.db.transaction() calls (merge, contact create/update) become savepoints of
// the outer transaction once wrapped, same as everywhere else in this rollout. Post-hoc
...
export async function accountRoutes(
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      let whereClause = and(
        eq(crmAccounts.tenantId, ctx.tenant.tenantId),
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const [account] = await ctx.db.raw
        .select()
        .from(crmAccounts)
        .where(and(eq(crmAccounts.id, id), eq(crmAccounts.tenantId, ctx.tenant.tenantId)));
      ...
      const rows = await ctx.db.raw
        ...
        .where(
          and(
            ...
            eq(crmAccountContacts.tenantId, ctx.tenant.tenantId)
          )
        )
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const [account] = await ctx.db.raw
        ...
        .from(crmAccounts)
        .where(and(eq(crmAccounts.id, accountId), eq(crmAccounts.tenantId, ctx.tenant.tenantId)));

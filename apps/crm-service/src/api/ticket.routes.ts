...
//
// Phase 9 GUC-per-request rollout — migrated 2026-08-21. No external I/O — TicketService has no
// fetch() calls. Post-hoc ctx.audit.log()/ctx.events.publish() calls are safe per caveat 4b.
...
export async function ticketRoutes(
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      let whereClause = eq(crmTickets.tenantId, ctx.tenant.tenantId);
      if (query.status) {
        whereClause = and(
          whereClause,
      ...
      if (query.mine === 'true') {
        whereClause = and(whereClause, eq(crmTickets.assignedTo, ctx.tenant.userId))!;
      }
      if (query.customerId) {
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const [ticket] = await ctx.db.raw
        .select()
        .from(crmTickets)
        .where(and(eq(crmTickets.id, id), eq(crmTickets.tenantId, ctx.tenant.tenantId)));
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const [updated] = await ctx.db.raw
        ...
        .where(
          and(
            eq(crmTickets.id, id),
            eq(crmTickets.tenantId, ctx.tenant.tenantId),
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const { id: idParam } = request.params as { id: string };
      const id = parseInt(idParam, 10);
      const updated = await TicketService.reopen(
        ctx.db.raw,
        ctx.tenant.tenantId,
        id,
        ctx.tenant.userId,
      ...
      const rows = await ctx.db.raw
        ...
        .where(
          and(
            ...
            eq(crmTicketMessages.tenantId, ctx.tenant.tenantId)
          )
        )

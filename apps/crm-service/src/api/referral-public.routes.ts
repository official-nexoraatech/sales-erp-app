...
// Phase 9 GUC-per-request rollout — deliberately NOT migrated. Unlike link-tracking.routes.ts
// (where the route itself does the lookup-then-scoped-write), ReferralService.trackClick()/
// redeem() are single functions that resolve the tenant from the code internally and do their
// own multi-step lookup+write on the plain db handle passed in — there's no discrete
// route-level query to wrap with withTenantConnection without restructuring the service methods
// themselves to split "resolve tenant" from "do the scoped work", which is a service-level design
// change, not a route-file swap. Left as explicit future work alongside caveat 4e's class of
// deferred cases.

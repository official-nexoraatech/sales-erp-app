...
import { tenantScopedHandler, getBranchScope, withTenantConnection } from '@erp/sdk';
...
// POST /leads/:id/convert is deliberately NOT migrated: LeadService.convertToCustomer() awaits a
// real fetch() call to sales-service's internal customer-create endpoint, interleaved with its
// own DB writes (checklist caveat 4). Every other route is safe: notifyLeadAssigned() in
// POST /leads/:id/assign is called with `void` (fire-and-forget, not awaited — caveat 4c), and
// POST /leads/capture is public but its tenantId is known upfront from the request body (unlike
// referral-public.routes.ts/link-tracking.routes.ts's own public routes), so it's wrapped with
// withTenantConnection directly rather than tenantScopedHandler (no req.auth here at all).
...
export async function leadRoutes(
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const { lead, duplicate } = await withTenantConnection(
        ctxFactory.rawDb,
        body.data.tenantId,
        async (db) => {
          const ctx = ctxFactory.create(
            { tenantId: body.data.tenantId, userId: 0, correlationId: crypto.randomUUID() },
            db
          );
          const result = await LeadService.capture(ctx.db.raw, body.data.tenantId, {
          ...
          if (!result.duplicate) {
            await ctx.events.publish(
              ...
              result.lead.id,
              'LEAD_CAPTURED',
              result.lead as unknown as Record<string, unknown>
            );
          }
          return result;
        }
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      let whereClause = eq(crmLeads.tenantId, ctx.tenant.tenantId);
      if (query.stage) {
        whereClause = and(
          whereClause,
      ...
      if (query.mine === 'true') {
        whereClause = and(whereClause, eq(crmLeads.assignedTo, ctx.tenant.userId))!;
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const [lead] = await ctx.db.raw
        .select()
        .from(crmLeads)
        .where(and(eq(crmLeads.id, id), eq(crmLeads.tenantId, ctx.tenant.tenantId)));
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const [existingOpen] = await ctx.db.raw
        ...
        .where(
          and(
            eq(crmLeads.tenantId, ctx.tenant.tenantId),
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      if (body.data.userId) {
        ...
        assignedUserId = await LeadService.autoAssign(
          ctx.db.raw,
          ctx.tenant.tenantId,
          id,
          existing.branchId
  ...
  // ── POST /leads/:id/convert — deliberately NOT migrated (see file-level comment) ───────────
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const rows = await ctx.db.raw
        ...
        .where(
          and(eq(crmLeadActivities.leadId, id), eq(crmLeadActivities.tenantId, ctx.tenant.tenantId))
        )
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const rows = await ctx.db.raw
        ...
        .where(eq(crmAssignmentRules.tenantId, ctx.tenant.tenantId));
      return reply.code(200).send({ data: { content: rows, totalElements: rows.length } });
    })
  );

 ...
 *
 * CRM/O2C split — moved here from sales-service (2026-08-20). The invoices read in
 * computeAndCacheSuggestions is a read-only cross-service Postgres read (revenue/order-count
 * comparison), not a transactional-atomicity requirement — same shared-table precedent as
 * customers/branches reads in migrations 1-3. businessSeasons (also written here via approve())
 * is not read or written anywhere else in the codebase, so there's no shared-consequence
 * concern the way LoyaltyService's split needed.
 */

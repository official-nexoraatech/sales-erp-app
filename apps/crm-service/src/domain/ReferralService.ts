...
// CRM/O2C split — LoyaltyService.creditPoints() stays in sales-service (pos.routes.ts calls it
// inside a POS sale's own transaction, an atomicity requirement this scheduler-driven caller
// doesn't share — see sales-service's internal.routes.ts POST /internal/loyalty/credit-points
// comment). Reached over HTTP, same x-internal-key + circuit-breaker convention used throughout
// this split.
...
async function creditPointsViaInternalApi(
  ...
  const res = await fetch(`${salesServiceUrl}/api/v2/internal/loyalty/credit-points`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-internal-key': internalKey },
    body: JSON.stringify(params),
  });
  if (!res.ok) throw new Error(`sales-service internal credit-points call failed: ${res.status}`);
  const json = (await res.json()) as { data: CreditPointsResult };
  return json.data;
}
const creditPointsBreaker = createCircuitBreaker(creditPointsViaInternalApi, 'sales-service');
...
export class ReferralService {
   ...
   * parties via sales-service's /internal/loyalty/credit-points endpoint (CRM/O2C split —
   * LoyaltyService.creditPoints() stays there, see this file's own header comment above).
   *
  ...
  static async attributeQualifyingPurchases(db: ErpDatabase, tenantId: number): Promise<number> {
    ...
    for (const reward of pending) {
      ...
      const referrerCredit = await creditPointsBreaker.fire(salesServiceUrl, internalKey, {
        tenantId,
        customerId: reward.referrerCustomerId,
        points: reward.referrerPoints,
        referenceType: 'REFERRAL',
        referenceId: reward.id,
        createdBy: 0,
        notes: `Referral reward for referring customer #${refereeCustomer.id}`,
      });
      const refereeCredit = await creditPointsBreaker.fire(salesServiceUrl, internalKey, {
        tenantId,
        customerId: refereeCustomer.id,
        points: reward.refereePoints,
        referenceType: 'REFERRAL',
        referenceId: reward.id,
        createdBy: 0,
        notes: `Referral welcome reward (referred by customer #${reward.referrerCustomerId})`,
      });

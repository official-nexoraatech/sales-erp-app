...
// reach customer B's data through any /portal/* route in crm-service's own portal.routes.ts
// (the Ticket/Referral half — moved here from sales-service, CRM/O2C split migration 12; the
// O2C half — orders/preferences/loyalty-bridge — stays in sales-service with its own equivalent
// boundary test), on a real database with two real seeded customers. A parameterized route
// table (not one-off tests per route) plus a meta-assertion checking the table's length against
// the route count scraped from portal.routes.ts, so a new :id-scoped route added without a
// matching boundary-test entry fails CI — same self-defending-scanner idea
...
import { branches, customers, crmTickets, crmTicketMessages } from '@erp/db';
...
describe.skipIf(!DB_URL)('portal routes (crm-service) — cross-customer boundary', () => {
  let db: ReturnType<typeof createDatabaseClient>;
  let app: FastifyInstance;
  let privateKey: KeyLike;
  const TEST_TENANT = 902_251 + Math.floor(Math.random() * 1000);
  let branchId: number;
  ...
  it('GET /portal/tickets/:id succeeds for the owning customer (positive control)', async () => {
    const token = await tokenFor(customerA);
    const res = await app.inject({
      method: 'GET',
      url: `/portal/tickets/${ticketA}`,
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(res.statusCode).toBe(200);
    expect(res.json().data.id).toBe(ticketA);
  });
});

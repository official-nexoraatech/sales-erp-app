...
//
// CRM/O2C split (migration 7) — this file was split: the describe blocks below (all calling
// campaignRoutes) moved here; sender-identity/integrations/communication-settings blocks stayed
// in sales-service's version of this file, since those routes didn't move.
...
describe('POST /crm/campaigns/:id/approve — requirePermission(CRM_CAMPAIGN_APPROVE)', () => {
  ...
  beforeAll(async () => {
    ...
    await campaignRoutes(app, mockCtxFactory);
  });
  afterAll(() => app.close());

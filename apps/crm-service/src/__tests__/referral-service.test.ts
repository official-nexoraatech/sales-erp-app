...
describe.skipIf(!DB_URL)('ReferralService — integration (CRM-ROADMAP Phase 2, Feature 4)', () => {
  ...
  beforeAll(async () => {
    ...
    // CRM/O2C split — attributeQualifyingPurchases's payout now calls sales-service's
    // POST /internal/loyalty/credit-points over HTTP (creditPointsBreaker in
    // ReferralService.ts) instead of calling LoyaltyService.creditPoints() in-process. This
    // suite is about ReferralService's own logic (fraud guardrails, funnel stats, payout
    // bookkeeping), not LoyaltyService's tier-evaluation correctness (covered separately in
    // each service's own LoyaltyService test), so the mock replicates only creditPoints'
    // externally-visible effect: bump customers.loyaltyPoints, insert a ledger row, return a
    // real transactionId — via this test's own db connection, same pattern as
    // lead-service.test.ts's customer-create mock.
    const originalFetch = globalThis.fetch;
    vi.stubGlobal('fetch', async (url: string | URL, init?: RequestInit) => {
      if (!String(url).includes('/internal/loyalty/credit-points')) return originalFetch(url, init);
      ...
      const balanceAfter = balanceBefore + body.points;
      const [txn] = await db
      ...
      await db
        ...
        .where(eq(customers.id, body.customerId));
      return new Response(
        JSON.stringify({ data: { points: body.points, transactionId: txn!.id } }),
        { status: 200 }
      );
    });

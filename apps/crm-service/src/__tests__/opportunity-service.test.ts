...
// reason), optimistic locking, and branch scoping (AR-6) for both a scoped and an unscoped
// caller. markWon's own outbox-event-publishing behavior is covered here (Won commits, the
// right event lands in outbox_events); what happens *after* that event is published — the
// async consumer creating the real Quotation, or failing on a blocked customer — is
// sales-service's concern now (CRM/O2C split) and is covered by
// opportunity-won-consumer.test.ts there instead.
...
describe.skipIf(!DB_URL)('OpportunityService — integration', () => {
  ...
  describe('markWon — publishes the outbox event (CRM/O2C split step 1)', () => {
    it('commits Won immediately with quotationId null, and publishes a correctly-shaped OPPORTUNITY_WON event', async () => {
      const opp = await OpportunityService.create(db, {
        tenantId: TEST_TENANT,
        name: 'Winnable Deal',
        value: 5_000,
      ...
      // Won commits synchronously; the quotation does not exist yet — the async consumer
      // (sales-service's OpportunityWonConsumer) creates it, tested there.
      expect(result.opportunity.stage).toBe('WON');
      expect(result.quotationId).toBeNull();
      expect(result.opportunity.convertedQuotationId).toBeNull();

      const [row] = await db
        .select()
        .from(outboxEvents)
        .where(
          and(eq(outboxEvents.eventType, 'OPPORTUNITY_WON'), eq(outboxEvents.aggregateId, opp.id))
        );
      expect(row).toBeDefined();
      const payload = row!.payload as {
        opportunityId: number;
        customerId: number;
        lines: Array<{ itemId: number; gstRate: number; hsnCode: string }>;
      };
      expect(payload.opportunityId).toBe(opp.id);
      expect(payload.customerId).toBe(customerId);
      // gstRate/hsnCode frozen fresh from `items` at Won-time, not re-derived by the consumer.
      expect(payload.lines).toHaveLength(1);
      expect(payload.lines[0]!.itemId).toBe(itemId);
      expect(payload.lines[0]!.gstRate).toBe(12);
      expect(payload.lines[0]!.hsnCode).toBe('5208');
    });

    it('markWon commits Won even for a blocked customer — the failure surfaces later, async, in the consumer', async () => {
      const opp = await OpportunityService.create(db, {
        tenantId: TEST_TENANT,
      ...
      // Won commits regardless — the customer-blocked failure hasn't happened yet (it happens
      // in sales-service's consumer, when it actually tries to create the Quotation).
      expect(result.opportunity.stage).toBe('WON');
      const [row] = await db
        .select()
        .from(outboxEvents)

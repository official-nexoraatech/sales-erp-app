    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      if (!existing) {
        return reply.code(200).send({
          data: {
            vendor: 'GENERIC_CSV',
            columnMapping: DEFAULT_BIOMETRIC_CONFIG.columnMapping,
            dateFormat: DEFAULT_BIOMETRIC_CONFIG.dateFormat,
          },
        });
      }
      ...
      const [result] = existing
        ...
        : await ctx.db.raw
            ...
            .values({
              ...
              createdBy: userId,
              ...body.data,
            } as typeof biometricDeviceConfigs.$inferInsert)
            .returning();

      if (!result) throw new Error('Biometric device config upsert failed');
      await ctx.audit.log({
        action: existing ? 'UPDATE' : 'CREATE',
        entityType: 'biometric_device_config',
        entityId: result.id,
      });

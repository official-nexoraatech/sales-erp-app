    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      if (!tailoringEnabled) {
        throw new BusinessError(
          'FEATURE_DISABLED',
          'Tailor work log feature (hr.tailoring.enabled) is disabled for this tenant'
        );
      }
  ...
  fastify.get(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      const conditions = [
        ...
        eq(tailorWorkLog.employeeId, query.data.employeeId),
      ];
      if (query.data.month) {

...
import { employees, leaveTypes, employeeLeaveBalance, alterationOrders } from '@erp/db';
import { and, eq, isNull, lt, ne } from 'drizzle-orm';
import { timingSafeEqual } from 'node:crypto';

// Full months between an employee's joining date and today — used to gate leave types like
// Maternity Leave (minServiceMonths: 6) from accruing before the employee is actually eligible.
function monthsOfService(joiningDate: string): number {
...
export async function internalRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/leave-applications/accrue-monthly', async (req, reply) => {
    ...
    for (const emp of activeEmployees) {
      const applicableTypes = allLeaveTypes.filter(
        (lt) => lt.tenantId === emp.tenantId && parseFloat(lt.daysPerYear) > 0
      );
      const serviceMonths = monthsOfService(emp.joiningDate);
      for (const lt of applicableTypes) {
        if (lt.isGenderSpecific && lt.genderAllowed && emp.gender !== lt.genderAllowed) continue;
        if (serviceMonths < lt.minServiceMonths) continue;
        ...
        const [existing] = await db
          ...
          .where(
            and(
              eq(employeeLeaveBalance.tenantId, emp.tenantId),
              eq(employeeLeaveBalance.employeeId, emp.id),
              eq(employeeLeaveBalance.leaveTypeId, lt.id),
        ...
        if (existing) {
          await db
            ...
            .set({
              totalDays: String(
                Math.min(
                  parseFloat(lt.daysPerYear),
                  parseFloat(existing.totalDays) + monthlyAccrual
                )
              ),
              updatedAt: new Date(),
            })
            .where(eq(employeeLeaveBalance.id, existing.id));
        } else {
  ...
  fastify.post('/leave-applications/year-end-carry-forward', async (req, reply) => {
    ...
    const balances = await db
      .select()
      .from(employeeLeaveBalance)
      .where(eq(employeeLeaveBalance.year, currentYear));
    ...
    return reply.send({
      data: { message: 'Year-end carry forward complete', carriedForward, expired },
    });
  });
  ...
  fastify.post('/seed-feature-flags', async (req, reply) => {
    ...
    await db
      .insert(featureFlags)
      .values({ tenantId: null, flagKey: 'hr.tailoring.enabled', enabled: true })
      .onConflictDoNothing();

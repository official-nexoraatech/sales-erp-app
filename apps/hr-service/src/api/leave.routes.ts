...
export async function leaveRoutes(
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const [emp] = await ctx.db.raw
        ...
        .from(employees)
        .where(and(eq(employees.id, body.data.employeeId), eq(employees.tenantId, tenantId)));
      if (!emp) throw new NotFoundError('Employee', body.data.employeeId);

      const [leaveType] = await ctx.db.raw
        .select()
        .from(leaveTypes)
        .where(and(eq(leaveTypes.id, body.data.leaveTypeId), eq(leaveTypes.tenantId, tenantId)));
      if (!leaveType) throw new NotFoundError('LeaveType', body.data.leaveTypeId);

      if (
        leaveType.isGenderSpecific &&
        leaveType.genderAllowed &&
        emp.gender !== leaveType.genderAllowed
      ) {
        throw new BusinessError(
          'LEAVE_GENDER_RESTRICTED',
          `${leaveType.name} is only applicable for ${leaveType.genderAllowed} employees`
        );
      }
      ...
      if (leaveType.requiresDocument) {
        ...
        if (days > leaveType.documentRequiredAfterDays && !body.data.documentUrl) {
          throw new ValidationError(
            `Document required for leave applications longer than ${leaveType.documentRequiredAfterDays} days`
          );
        }
      }
      ...
      const [balance] = await ctx.db.raw
        ...
        .where(
          and(
            eq(employeeLeaveBalance.tenantId, tenantId),
            eq(employeeLeaveBalance.employeeId, body.data.employeeId),
            eq(employeeLeaveBalance.leaveTypeId, body.data.leaveTypeId),
            eq(employeeLeaveBalance.year, year)
          )
        );
      ...
      if (balance) {
        const available =
          ...
          parseFloat(balance.carriedForwardDays) -
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const [app] = await ctx.db.raw
        ...
        .where(and(eq(leaveApplications.id, id), eq(leaveApplications.tenantId, tenantId)));
      ...
      // Atomically claim the row (see StockAdjustmentService.approve()/payroll approve()/
      // disburse() for the same pattern) — a plain get()-then-update() status check let two
      // concurrent approve calls on the same application both pass before either committed,
      // each then adding the full day-count to usedDays inside its own transaction and double-
      // deducting the employee's leave balance for one physical approval.
      await ctx.db.transaction(async (trx) => {
        ...
        if (!claimed) {
          ...
          throw new BusinessError(
            'LEAVE_NOT_PENDING',
            `Leave application is ${current?.status ?? 'unknown'}, cannot approve`
          );
        }
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (app.status !== 'PENDING')
        throw new BusinessError(
          'LEAVE_NOT_PENDING',
          `Leave application is ${app.status}, cannot reject`
        );
      ...
      await ctx.audit.log({
        ...
        entityType: 'leave_application',
        entityId: id,
        metadata: { action: 'REJECT', reason: body.data.rejectionReason },
      });
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (new Date(app.startDate) <= new Date())
        throw new BusinessError(

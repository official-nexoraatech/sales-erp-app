...
async function schedulerRequest(
  ...
  if (!res.ok)
    throw new BusinessError(
      ...
      json.error?.message ?? `Scheduler request failed: ${path}`
    );
  return json;
}
...
// Resolves the tenant-configurable standard-hours threshold for overtime: the employee's
// assigned shift, falling back to the tenant's default shift, falling back to 8 hours if
// neither exists (e.g. the tenant hasn't set up shifts at all yet).
async function resolveStandardHours(
  ...
  const [shift] =
    shiftId != null
      ? await db.raw
          .select({ standardHours: shifts.standardHours })
          .from(shifts)
  ...
  return shift ? Number(shift.standardHours) : 8;
}
...
function computeWorkHours(
  ...
  checkOut?: string,
  standardHours = 8
): { workHours: number; overtimeHours: number; isLate: boolean } {
...
export async function attendanceRoutes(
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const standardHours = await resolveStandardHours(ctx.db, tenantId, emp.shiftId);
      const { workHours, overtimeHours } = computeWorkHours(
        body.data.checkInTime,
        body.data.checkOutTime,
        standardHours
      );

      const upserted = await ctx.db.transaction(async (trx) => {
        ...
        if (row) {
          ...
          await eventBus.publishInTransaction('attendance', row.id, 'ATTENDANCE_MARKED', {
            ...row,
      ...
      await ctx.audit.log({
        ...
        entityId: upserted?.id ?? 0,
        metadata: {
          employeeId: body.data.employeeId,
          date: body.data.attendanceDate,
          status: body.data.status,
        },
      });
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      // Verify every referenced employee exists (unlike the single-employee /attendance/mark
      // route, this bulk path previously inserted attendance rows for any employeeId with no
      // existence check at all) and batch-resolve each one's shift for the overtime threshold,
      // mirroring the /attendance/import route's resolution pattern.
      const employeeIds = [...new Set(body.data.records.map((r) => r.employeeId))];
      const employeeRows = await ctx.db.raw
        .select({ id: employees.id, shiftId: employees.shiftId })
        .from(employees)
        .where(
          and(
            eq(employees.tenantId, tenantId),
      ...
      if (missingIds.length > 0) throw new NotFoundError('Employee', missingIds[0] as number);
      ...
      for (const record of body.data.records) {
        ...
        const standardHours = shift ? Number(shift.standardHours) : 8;
        const { workHours, overtimeHours } = computeWorkHours(
          record.checkInTime,
          record.checkOutTime,
          standardHours
  ...
  fastify.put<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const [existing] = await ctx.db.raw
        .select()
        .from(attendance)
        .where(and(eq(attendance.id, id), eq(attendance.tenantId, tenantId)));
      ...
      const updated = await ctx.db.transaction(async (trx) => {
        const [row] = await trx.raw
          .update(attendance)
          .set(updates as unknown as Partial<typeof attendance.$inferInsert>)
          .where(eq(attendance.id, id))
          .returning();
        if (row) {
          const eventBus = new PlatformEventBus(trx, tenantId, userId, ctx.tenant.correlationId);
          await eventBus.publishInTransaction('attendance', row.id, 'ATTENDANCE_CORRECTED', {
            ...row,
            employeeName: emp?.displayName,
          } as unknown as Record<string, unknown>);
        }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (!ALLOWED_PUNCH_MIME_TYPES.has(file.mimetype))
      ...
      const uploadRes = await schedulerRequest('/imports/upload', 'POST', authHeader, {
        entityType: 'attendance',
        csvData,
      ...
      if (!jobId)
        throw new BusinessError(
          'IMPORT_UPLOAD_FAILED',
          'Scheduler service did not return a job id'
        );
      ...
      await schedulerRequest(`/imports/${jobId}/map`, 'POST', authHeader, {
        mappings: fields.map((f) => ({ sourceColumn: f, targetField: f })),
      });
    ...
    async (request, reply) => {
      ...
      for (const row of rows) {
        if (!summary[row.employeeId])
          summary[row.employeeId] = { presentDays: 0, absentDays: 0, lopDays: 0, lateDays: 0 };

...
// Phase 9 GUC-per-request rollout — deliberately NOT migrated. `performance_profiles`
// (packages/db-client/src/schema/distributed.ts) has no tenant_id column at all — P95/P99
// latency benchmarks are per-endpoint, platform-wide metrics shared across every tenant's
// traffic, not a per-tenant record. Same "no tenant dimension at all" category as
// schema-registry.routes.ts in this same service.
  ...
  fastify.get('/admin/performance/baselines', {
    ...
    handler: async (request, reply) => {
      ...
      const result = (
        ...
        meetsTarget:
          r.p95_ms !== null && r.target_p95_ms !== null ? r.p95_ms <= r.target_p95_ms : null,
      }));

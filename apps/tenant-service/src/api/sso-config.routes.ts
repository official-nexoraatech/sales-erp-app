...
export async function ssoConfigRoutes(
  ...
  // stripped.
  fastify.get('/sso-config', { preHandler: [authenticate, requirePermission(PERMISSIONS.SSO_CONFIG_MANAGE)] }, async (request, reply) => {
    const { tenantId } = request.auth;
    const ctx = ctxFor(request);

    const [config] = await ctx.db.raw.select().from(ssoConfigs).where(eq(ssoConfigs.tenantId, tenantId));
  ...
  // ── PUT /sso-config ───────────────────────────────────────────────────────
  fastify.put('/sso-config', { preHandler: [authenticate, requirePermission(PERMISSIONS.SSO_CONFIG_MANAGE)] }, async (request, reply) => {
    ...
    const [existing] = await ctx.db.raw.select().from(ssoConfigs).where(eq(ssoConfigs.tenantId, tenantId));

    if (!existing) {
      if (!clientSecret) {
        throw new ValidationError('clientSecret is required when configuring SSO for the first time');
      }
      const [created] = await ctx.db.raw
        ...
        .returning();
      if (created) {
        await ctx.events.publish('sso-config', tenantId, 'SSO_CONFIG_UPDATED', { tenantId, provider: created.provider, enabled: created.enabled });
      }
    ...
    const [updated] = await ctx.db.raw
      ...
      .set({
        ...fields,
        ...(clientSecret ? { clientSecretEncrypted: encryptField(clientSecret, requireEnv('FIELD_ENCRYPTION_KEY')) } : {}),
        updatedAt: new Date(),
        updatedBy: userId,
    ...
    if (updated) {
      await ctx.events.publish('sso-config', tenantId, 'SSO_CONFIG_UPDATED', { tenantId, provider: updated.provider, enabled: updated.enabled });
    }
  ...
  fastify.delete('/sso-config', { preHandler: [authenticate, requirePermission(PERMISSIONS.SSO_CONFIG_MANAGE)] }, async (request, reply) => {
    const { tenantId } = request.auth;
    const ctx = ctxFor(request);

    const [deleted] = await ctx.db.raw.delete(ssoConfigs).where(eq(ssoConfigs.tenantId, tenantId)).returning();

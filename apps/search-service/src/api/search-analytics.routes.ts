...
export async function searchAnalyticsRoutes(fastify: FastifyInstance, db: ErpDatabase): Promise<void> {
  fastify.post('/search/analytics/click', { preHandler: [authenticate] }, async (request, reply) => {
  ...
  fastify.get('/admin/search/analytics/summary', { preHandler: [authenticate] }, async (request, reply) => {

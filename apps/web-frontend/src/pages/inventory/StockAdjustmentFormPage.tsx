...
export default function StockAdjustmentFormPage() {
  ...
  function addLine(item: Item) {
    ...
    // The backend requires unitCost to be either omitted or strictly > 0 — a truthy-string
    // check on purchasePrice ("0.00" is truthy) used to prefill 0 for any item that has no
    // purchase price recorded yet, which then always failed adjustment creation with a 500.
  ...
  return (
    <div>
      <ERPPageHeader
        ...
        subtitle="Record stock gain or loss"
                ...
                <tr key={line.itemId}>
                  ...
                  <td className="py-2">
                    <select
                      ...
                      className="text-xs rounded border-default bg-surface-card px-1 py-1"
                    >

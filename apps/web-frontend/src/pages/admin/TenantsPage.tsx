...
export default function TenantsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['admin-tenants'], queryFn: () => adminTenantApi.list() });
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<ReasonForm>();
  ...
  const columns: ERPColumnDef<Tenant>[] = [
    ...
    },
    {
      ...
      render: (r) => {
        ...
        if (r.status === 'ACTIVE') {
          items.push({ label: 'Suspend', icon: Ban, variant: 'danger', onClick: () => openReasonDialog('suspend', r) });
        }
        if (r.status === 'SUSPENDED') {
          items.push({ label: 'Activate', icon: CheckCircle, onClick: () => activateMutation.mutate(r.id) });
        }
        if (r.status !== 'CLOSED') {
          items.push({ label: 'Close', icon: XCircle, variant: 'danger', onClick: () => openReasonDialog('close', r) });
        }
  ...
  return (
    <div>
      <ERPPageHeader variant="list" title="Tenants" subtitle="Manage every organization provisioned on this platform." />
      <ERPDataGrid columns={columns} data={tenantsList} isLoading={isLoading} rowKey="id" />

      <Modal
        open={!!dialog}
        onClose={() => setDialog(null)}
        title={dialog?.mode === 'suspend' ? `Suspend ${dialog.tenant.name}` : `Close ${dialog?.tenant.name ?? ''}`}
        size="sm"
      >
        <form onSubmit={handleSubmit((d) => reasonMutation.mutate(d))} className="space-y-4">
          <Input
            ...
            {...register('reason', { required: 'Required', minLength: { value: 5, message: 'At least 5 characters' } })}
            error={errors.reason?.message}
          />
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="secondary" type="button" onClick={() => setDialog(null)}>Cancel</Button>
            <Button type="submit" variant="danger" loading={isSubmitting || reasonMutation.isPending}>

...
export default function PerformancePage() {
  ...
  const baselines: PerformanceBaseline[] = (baselinesData as unknown as PerformanceBaseline[]) ?? [];
  ...
  return (
    <div className="space-y-6">
      <ERPPageHeader variant="list" title="Performance Baselines" subtitle="P50/P95/P99 latency measurements vs. targets" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="card p-4">
          ...
          <p className={`text-2xl font-bold ${breachCount > 0 ? 'text-danger' : 'text-success'}`}>{breachCount}</p>
        </div>
        <div className="card p-4">
        ...
        {baselinesLoading ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-surface-hover">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">Endpoint</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">Target</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">P50</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">P95</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">P99</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">Samples</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">Status</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-secondary uppercase">Measured</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {baselines.map((b) => {
                const target = targetMap.get(`${b.method}:${b.endpoint}`);
                const variant = target !== undefined ? statusVariant(b.p95Ms, target) : 'default' as const;
                return (
                  <tr key={`${b.method}:${b.endpoint}`} className="hover:bg-surface-hover">
                    ...
                    <td className="px-4 py-3 text-secondary">{target !== undefined ? formatMs(target) : '—'}</td>
                    <td className="px-4 py-3 text-primary">{formatMs(b.p50Ms)}</td>
                    <td className={`px-4 py-3 font-semibold ${variant === 'success' ? 'text-success' : variant === 'warning' ? 'text-warning' : variant === 'danger' ? 'text-danger' : 'text-primary'}`}>
                      {formatMs(b.p95Ms)}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {target !== undefined ? (
                        <Badge variant={variant}>{variant === 'success' ? 'OK' : variant === 'warning' ? 'SLOW' : 'BREACH'}</Badge>
                      ) : (
      ...
      {targets.length > 0 && (
        <div className="card p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {targets.map((t) => (
              <div key={`${t.method}:${t.endpoint}`} className="px-3 py-2 rounded-lg bg-surface-hover">
                <p className="text-xs text-secondary">{t.method} {t.endpoint}</p>

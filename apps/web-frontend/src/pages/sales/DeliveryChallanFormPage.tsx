...
import { deliveryChallanApi, itemApi, customerApi, warehouseApi, branchApi } from '../../api/endpoints.js';
...
export default function DeliveryChallanFormPage() {
  ...
  const { data: customerData } = useQuery({ queryKey: ['customers-list'], queryFn: () => customerApi.list({}), enabled: hasPermission(PERMISSIONS.CUSTOMER_VIEW) });
  ...
  const itemOptions = (itemData as { content?: Array<{ id: number; name: string; hsnCode?: string; minSalePrice?: string }> })?.content ?? [];
  ...
  return (
    <div>
      <ERPPageHeader variant="list" title="New Delivery Challan" subtitle="Record goods dispatched to a customer before invoicing" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        />
        <Input label="Challan Date" required type="date" value={challanDate} onChange={(e) => setChallanDate(e.target.value)} />
      </div>
          ...
          <tbody className="divide-y dark:divide-gray-700">
            {computedLines.map((l, idx) => (
              <tr key={idx}>
                ...
                <td className="py-2 pr-2">
                  <input type="text" value={l.hsnCode}
                    onChange={(e) => updateLine(idx, 'hsnCode', e.target.value)}
                    className="w-24 rounded border-default bg-surface-card px-2 py-1 text-sm" />
                </td>
                <td className="py-2 pr-2 text-right font-semibold">{formatCurrency(l.lineTotal)}</td>
                <td className="py-2">
            ...
            {lines.length === 0 && (
              <tr><td colSpan={6} className="py-6 text-center text-disabled text-sm">No items added yet</td></tr>
            )}
          </tbody>
        </table>
      ...
      <div className="mb-6">
        <ERPTextarea label="Notes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Any additional notes for this challan…" />
      </div>

      <div className="flex justify-end gap-3">
        <Button variant="ghost" onClick={() => navigate('/sales/delivery-challans')}>Cancel</Button>

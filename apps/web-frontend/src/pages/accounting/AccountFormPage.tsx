...
import { accountFormSchema, ACCOUNT_TYPES, type AccountFormData } from '../../schemas/account.schema.js';
import { useDirtyFormGuard } from '../../hooks/useDirtyFormGuard.js';

interface AccountOption { id: number; accountCode: string; name: string; }
...
export default function AccountFormPage() {
  ...
  const { data: accountsData } = useQuery({ queryKey: ['accounts'], queryFn: () => accountApi.list() });
  const accounts = ((accountsData as Record<string, unknown[]>)?.content ?? []) as AccountOption[];

  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting, isDirty } } = useForm<AccountFormData>({
    ...
    <div>
      ...
      <form onSubmit={handleSubmit((d) => mutation.mutate(d as unknown as Record<string, unknown>))} className="space-y-6" noValidate>
        <ERPFormSection title="Account Details" columns={2}>
          <Input label="Account Code" required {...register('accountCode')} error={errors.accountCode?.message} />
          <Input label="Account Name" required {...register('name')} error={errors.name?.message} />
          <Select label="Account Type" required {...register('accountType')} error={errors.accountType?.message}>
            <option value="">Select…</option>
            {ACCOUNT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
          </Select>
          <Select label="Normal Balance" required {...register('normalBalance')} error={errors.normalBalance?.message}>
            <option value="">Select…</option>
            <option value="DEBIT">DEBIT</option>
            <option value="CREDIT">CREDIT</option>
          </Select>
          <Select label="Parent Account" {...register('parentId')} error={errors.parentId?.message} hint="Leave blank for a root account">
            <option value="">None</option>
            {accounts.filter((a) => String(a.id) !== id).map((a) => <option key={a.id} value={a.id}>{a.accountCode} — {a.name}</option>)}
          </Select>
          <Input label="Opening Balance (₹)" type="number" step="0.01" {...register('openingBalance')} error={errors.openingBalance?.message} />
        </ERPFormSection>

        <ERPFormSection title="Bank Details" description="Optional — only needed for cash/bank ledger accounts" columns={2}>
          ...
          {isBank && (
            <>
              ...
              <Input label="Account Number" {...register('bankAccountNo')} error={errors.bankAccountNo?.message} hint="Stored encrypted" />
            </>
          )}
        </ERPFormSection>

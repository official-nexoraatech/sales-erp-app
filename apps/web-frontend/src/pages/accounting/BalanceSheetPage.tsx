...
function SectionTable({ accounts, label, total }: { accounts: BSAccountRow[]; label: string; total: number }) {
  return (
    <>
      {accounts.map((a) => (
        <tr key={a.accountId} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750">
          <td className="px-4 py-2 pl-8 text-sm font-mono text-xs text-disabled w-24">{a.accountCode}</td>
      ...
      <tr className="bg-gray-50 dark:bg-gray-900/30 border-b-2 border-gray-300 dark:border-gray-600">
        <td className="px-4 py-2.5 text-sm font-semibold text-primary" colSpan={2}>{label} Total</td>
        <td className="px-4 py-2.5 text-right font-mono font-semibold text-primary">{formatCurrency(total)}</td>
      </tr>
    </>
  );
}
    ...
    <div className="space-y-6">
      ...
      {bs && (
        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium ${bs.isBalanced ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300'}`}>
          ...
          <span className="ml-auto font-normal text-secondary">Assets: {formatCurrency(bs.totalAssets)} | L+E: {formatCurrency(bs.totalLiabilitiesAndEquity)}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
          ...
          {isLoading ? <ERPTableSkeleton rows={6} /> : bs ? (
            <table className="w-full"><tbody><SectionTable accounts={bs.assets} label="Assets" total={bs.totalAssets} /></tbody></table>
          ) : null}
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
            ...
            {isLoading ? <ERPTableSkeleton rows={4} /> : bs ? (
              <table className="w-full"><tbody><SectionTable accounts={bs.liabilities} label="Liabilities" total={bs.totalLiabilities} /></tbody></table>
            ) : null}
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
            ...
            {isLoading ? <ERPTableSkeleton rows={3} /> : bs ? (
              <table className="w-full"><tbody><SectionTable accounts={bs.equity} label="Equity" total={bs.totalEquity} /></tbody></table>
            ) : null}
          </div>
        </div>
      </div>

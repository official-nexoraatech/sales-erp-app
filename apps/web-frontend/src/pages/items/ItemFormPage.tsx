...
export default function ItemFormPage() {
  ...
  const { data: itemData, isLoading: itemLoading } = useQuery({ queryKey: ['items', id], queryFn: () => itemApi.getById(Number(id)), enabled: isEdit });
  ...
  const { data: unitData } = useQuery({ queryKey: ['units'], queryFn: () => unitApi.list(), enabled: hasPermission(PERMISSIONS.UNIT_VIEW) });
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors, isSubmitting } } = useForm<ItemFormData>({
  ...
  if (isEdit && itemLoading) {
    return (
      <div>
        <ERPPageHeader variant="list" title="Edit Item" subtitle="Add or edit item master details." />
        <ERPFormSkeleton />
      </div>
    );
  }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit((d) => mutation.mutate(d as unknown as Record<string, unknown>))} className="space-y-6" noValidate>
        <ERPFormSection title="Basic Information" columns={2}>
          ...
          <Input label="Item Code / SKU" {...register('itemCode')} hint="Auto-generated if blank" error={errors.itemCode?.message} />
          ...
          <Select label="Unit of Measure" required {...register('unitId')} error={errors.unitId?.message}>
            <option value="">Select unit…</option>
            {(units as Record<string, unknown>[]).map((u) => <option key={u.id as number} value={u.id as number}>{u.name as string} ({u.abbreviation as string})</option>)}
          </Select>
        ...
        <ERPFormSection title="GST & HSN" columns={2}>
          ...
          <Select label="GST Rate %" required {...register('gstRate', { valueAsNumber: true })} error={errors.gstRate?.message}>
            {GST_RATES.map((r) => <option key={r} value={r}>{r}%</option>)}
          </Select>
          <Input label="Cess Rate %" type="number" step="0.01" {...register('cessRate')} error={errors.cessRate?.message} />
        </ERPFormSection>

        <ERPFormSection title="Pricing" columns={2}>
          <Input label="MRP (₹)" type="number" step="0.01" {...register('mrp')} error={errors.mrp?.message} />
          <Input label="Sale Price (₹)" type="number" step="0.01" {...register('salePrice')} error={errors.salePrice?.message} />
          <Input label="Min Sale Price (₹)" type="number" step="0.01" {...register('minSalePrice')} error={errors.minSalePrice?.message} />
        ...
        <ERPFormSection title="Inventory & Barcode" columns={2}>
          ...
          {isFabric && <Input label="Fabric Width (cm)" type="number" {...register('fabricWidth')} error={errors.fabricWidth?.message} />}

...
const LIST_PATH = '/inventory/brands';
...
export default function BrandFormPage() {
  ...
  const brand = isEdit ? brands.find((b) => b.id === Number(id)) : undefined;
  ...
  const mutation = useMutation({
    mutationFn: (d: Record<string, unknown>) =>
      isEdit ? brandApi.update(Number(id), d) : brandApi.create(d),
    onSuccess: () => {
  ...
  if (isEdit && isLoading) {
    return (
      <div>
        <ERPPageHeader variant="detail" title="Edit Brand" backTo={LIST_PATH} />
        <ERPFormSkeleton />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <form
        ...
        noValidate
      >
        <ERPFormSection title="Brand Details" columns={2}>
          <Input
            label="Name"
            required

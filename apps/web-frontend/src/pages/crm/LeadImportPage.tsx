...
// CRM-ROADMAP Phase 1, Feature 7 (Data Import/Dedupe/Merge Tooling) — same pattern as
// CrmAccountImportPage.tsx/SupplierImportPage.tsx. Dedupe warnings here flag a CSV row whose
// phone matches either an existing lead or an existing customer (ImportEngine's
// findLeadDuplicateWarnings) — never blocking, just surfaced before commit.

const TARGET_FIELDS = ['displayName', 'companyName', 'phone', 'email', 'source', 'isB2b'];
...
export default function LeadImportPage() {
  ...
  const handleDownloadTemplate = async () => {
    const blob = await importApi.template('lead');
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      setResult(res);
      toast.success(`Imported ${res.successRows} of ${res.totalRows} leads`);
    } catch (e) {
  ...
  return (
    <div>
      <ERPPageHeader
        variant="detail"
        title="Import Leads"
        subtitle="Bulk-create leads from a CSV file"
        backTo="/crm/leads"
      />

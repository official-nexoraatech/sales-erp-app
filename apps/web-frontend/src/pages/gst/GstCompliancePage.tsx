...
const STATUS_CONFIG: Record<FilingStatus, { label: string; color: string; icon: ReactNode }> = {
  FILED: { label: 'Filed', color: 'text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30', icon: <CheckCircle className="w-3.5 h-3.5" /> },
  LATE_FILED: { label: 'Late Filed', color: 'text-amber-700 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30', icon: <AlertTriangle className="w-3.5 h-3.5" /> },
  PENDING: { label: 'Pending', color: 'text-red-700 dark:text-red-400 bg-red-100 dark:bg-red-900/30', icon: <Clock className="w-3.5 h-3.5" /> },
  NOT_DUE: { label: 'Not Due', color: 'text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800', icon: <Clock className="w-3.5 h-3.5" /> },
};
...
function MarkFiledModal({ entry, onClose }: { entry: Record<string, unknown>; onClose: () => void }) {
  const [refNo, setRefNo] = useState('');
  const qc = useQueryClient();

  const mutation = useMutation({
    mutationFn: () => gstApi.markFiled(String(entry.returnType ?? ''), String(entry.period ?? ''), refNo || undefined),
    onSuccess: () => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Reference Number (optional)</label>
            <input
              type="text"
              value={refNo}
          ...
          <div className="flex gap-3 justify-end">
            ...
            >
              {mutation.isPending ? 'Saving...' : 'Confirm Filed'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">GST Compliance Calendar</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Return filing tracker &amp; e-Way Bill monitoring</p>
        </div>
      </div>
      ...
      {statusData && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            { key: 'pendingCount', label: 'Pending Returns', color: 'text-amber-600 dark:text-amber-400', icon: <Clock className="w-5 h-5" /> },
            { key: 'overdueCount', label: 'Overdue', color: 'text-red-600 dark:text-red-400', icon: <AlertTriangle className="w-5 h-5" /> },
            { key: 'filedThisMonth', label: 'Filed This Month', color: 'text-green-600 dark:text-green-400', icon: <CheckCircle className="w-5 h-5" /> },
            { key: 'nextDue', label: 'Next Due', color: 'text-indigo-600 dark:text-indigo-400', icon: <TrendingUp className="w-5 h-5" /> },
            ...
            <div key={key} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
              ...
              <div className={`text-lg font-semibold ${key === 'nextDue' ? 'text-sm font-medium' : ''} ${color}`}>
      ...
      {calLoading ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          {[
            ...
            <div key={label} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl">
                    ...
                    {entries.length === 0 ? (
                      ...
                      <tr key={i} className={`${e.isOverdue ? 'bg-red-50/30 dark:bg-red-900/5' : ''} hover:bg-gray-50 dark:hover:bg-gray-700/30`}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
                          {String(e.dueDate ?? '')}
                          {Boolean(e.isOverdue) && (
                            <span className="ml-1 text-xs text-red-600 dark:text-red-400">({String(e.daysOverdue ?? 0)}d late)</span>
                          )}
                        </td>
                        <td className="px-4 py-3"><StatusBadge status={String(e.status ?? 'PENDING')} /></td>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-sm font-semibold text-amber-800 dark:text-amber-300">
              {ewbData.totalElements} e-Way Bill{ewbData.totalElements > 1 ? 's' : ''} Expiring Within 24 Hours
            </h3>
          </div>
          <div className="space-y-2">
            {(ewbData.content as Record<string, unknown>[]).map((e, i) => (
              <div key={i} className="flex items-center justify-between text-sm">
                <span className="text-amber-700 dark:text-amber-400 font-mono">EWB: {String(e.ewbNumber ?? '')}</span>
                <span className="text-amber-600 dark:text-amber-500">Expires: {String(e.ewbValidUpto ?? '')}</span>
              </div>
            ))}
          </div>
        </div>

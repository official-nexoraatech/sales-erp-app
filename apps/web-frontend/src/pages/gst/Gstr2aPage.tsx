...
const STATUS_CONFIG: Record<ReconcStatus, { label: string; color: string; icon: React.ReactNode; action: string }> = {
  MATCHED: { label: 'Matched', color: 'text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30', icon: <CheckCircle className="w-3.5 h-3.5" />, action: 'No action needed' },
  BOOKS_ONLY: { label: 'Books Only', color: 'text-blue-700 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30', icon: <Info className="w-3.5 h-3.5" />, action: 'Contact supplier to file GSTR-1' },
  GSTR2A_ONLY: { label: 'GSTR-2A Only', color: 'text-purple-700 dark:text-purple-400 bg-purple-100 dark:bg-purple-900/30', icon: <Info className="w-3.5 h-3.5" />, action: 'Check if GRN was missed' },
  AMOUNT_MISMATCH: { label: 'Amount Mismatch', color: 'text-amber-700 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30', icon: <AlertTriangle className="w-3.5 h-3.5" />, action: 'Raise debit note or amend GRN' },
  UNMATCHED: { label: 'Unmatched', color: 'text-red-700 dark:text-red-400 bg-red-100 dark:bg-red-900/30', icon: <XCircle className="w-3.5 h-3.5" />, action: 'Manual review required' },
};
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">GSTR-2A Reconciliation</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Match purchase books against supplier-filed GSTR-2A data</p>
          </div>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {summaryCards.map(({ key, label, color }) => (
            <div key={key} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 text-center">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-900/50">
              <tr>
                {['Supplier GSTIN', 'Invoice No', 'Date', 'Taxable', 'GST Total', 'Status', 'Variance'].map((h) => (
              ...
              {activeTab === 'gstr2a' ? (
                gstr2aEntries.length === 0 ? (
                  ...
                  gstr2aEntries.map((e, i) => {
                    ...
                    const totalGst = Number(r.cgstAmount ?? 0) + Number(r.sgstAmount ?? 0) + Number(r.igstAmount ?? 0);
                    return (
                      <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td className="px-4 py-3 text-xs font-mono text-gray-700 dark:text-gray-300">{String(r.supplierGstin ?? '')}</td>
                        <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{String(r.invoiceNumber ?? '')}</td>
                        <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">{String(r.invoiceDate ?? '')}</td>
                        ...
                        <td className="px-4 py-3"><StatusBadge status={String(r.reconciliationStatus ?? 'UNMATCHED')} /></td>
                ...
                booksOnlyEntries.length === 0 ? (
                  ...
                  booksOnlyEntries.map((e, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td className="px-4 py-3 text-xs font-mono text-gray-700 dark:text-gray-300">{String(r.gstinOfCounterparty ?? '—')}</td>
                        <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{String(r.documentNumber ?? '')}</td>
                        <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{String(r.documentDate ?? '')}</td>
                        <td className="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{formatCurrency(r.taxableAmount)}</td>
                        <td className="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{formatCurrency(totalGst)}</td>

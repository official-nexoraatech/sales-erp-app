...
export default function PFChallanPage() {
  ...
  const markFiledMutation = useMutation({
    ...
    onSuccess: () => { toast.success('PF challan marked as filed'); qc.invalidateQueries({ queryKey: ['pf-challan', month, year] }); },
        ...
        actions={
          <div className="flex gap-2">
            {hasPermission(PERMISSIONS.HR_STATUTORY) && (
              <Button variant="secondary" onClick={handleExport} disabled={!challan?.rows.length}>Download for EPFO Portal</Button>
            )}
        ...
        <table className="w-full text-sm bg-surface-card rounded-xl border border-default overflow-hidden">
          ...
          <tbody className="divide-y divide-default">
            {challan.rows.map((r) => (
              <tr key={r.employeeId}>
                ...
                <td className="px-4 py-3 text-right font-mono">{formatCurrency(r.basicSalary)}</td>
                <td className="px-4 py-3 text-right font-mono">{formatCurrency(r.epfEmployee)}</td>
                <td className="px-4 py-3 text-right font-mono">{formatCurrency(r.epfEmployer)}</td>
          ...
          <tfoot className="bg-surface-subtle font-semibold">
            <tr>
              ...
              <td className="px-4 py-3 text-right font-mono">{formatCurrency(challan.totals.basicSalary)}</td>
              <td className="px-4 py-3 text-right font-mono">{formatCurrency(challan.totals.epfEmployee)}</td>
              <td className="px-4 py-3 text-right font-mono">{formatCurrency(challan.totals.epfEmployer)}</td>
              <td className="px-4 py-3 text-right font-mono">{formatCurrency(challan.totals.epsAmount)}</td>
            </tr>
          </tfoot>
        </table>
      )}

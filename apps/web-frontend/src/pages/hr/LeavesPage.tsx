...
interface LeaveType { id: number; name: string; daysPerYear: string; }
...
export default function LeavesPage() {
  ...
  const leaveTypes: LeaveType[] = ((typeData as Record<string, unknown>)?.content as LeaveType[]) ?? [];

  const seedMutation = useMutation({
    mutationFn: () => leaveApi.seedTypes(),
    onSuccess: () => { toast.success('Default leave types seeded'); qc.invalidateQueries({ queryKey: ['leave-types'] }); },
  ...
  const pending: LeaveApplication[] = ((pendingData as Record<string, unknown>)?.content as LeaveApplication[]) ?? [];

  const applyMutation = useMutation({
    mutationFn: () => leaveApi.apply({ employeeId: Number(employeeId), leaveTypeId: Number(leaveTypeId), startDate, endDate, reason }),
    onSuccess: () => {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Leave approved'); qc.invalidateQueries({ queryKey: ['leave-pending'] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  const rejectMutation = useMutation({
    mutationFn: (id: number) => leaveApi.reject(id, { rejectionReason: 'Rejected by manager' }),
    onSuccess: () => { toast.success('Leave rejected'); qc.invalidateQueries({ queryKey: ['leave-pending'] }); },
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-surface-card rounded-xl border border-default p-5 space-y-4">
          ...
          <Select label="Leave Type" value={leaveTypeId} onChange={(e) => setLeaveTypeId(e.target.value)}>
            ...
            {leaveTypes.map((t) => <option key={t.id} value={t.id}>{t.name} ({t.daysPerYear} days/yr)</option>)}
          </Select>
                  ...
                  <li key={app.id} className="border border-default rounded-lg p-3">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-secondary">{typeName(app.leaveTypeId)} — {app.days} day(s)</p>
                        <p className="text-xs text-disabled">{formatDate(app.startDate)} – {formatDate(app.endDate)}</p>
                      </div>
                      <Badge variant={STATUS_VARIANT[app.status] ?? 'default'}>{app.status}</Badge>
                    </div>
                    <div className="flex gap-2 mt-3">
                      <Button size="sm" onClick={() => approveMutation.mutate(app.id)} loading={approveMutation.isPending}>Approve</Button>
                      <Button size="sm" variant="danger-outline" onClick={() => rejectMutation.mutate(app.id)} loading={rejectMutation.isPending}>Reject</Button>
                    </div>
                  </li>
                ))}
              </ul>

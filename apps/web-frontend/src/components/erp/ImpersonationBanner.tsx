...
export default function ImpersonationBanner() {
  ...
  async function stop(auto: boolean) {
    ...
    // Cached queries were fetched as the impersonated user (see WEB-FRONTEND-AUDIT-2026-07-24.md,
    // Critical #1) — clear them so the restored admin session doesn't render stale data.
    queryClient.clear();
    setStopping(false);
    ...
    <div className="flex items-center justify-between gap-4 px-4 py-2 bg-amber-500 text-black text-sm font-medium shrink-0">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span className="truncate">
          ...
          ({user.email}) — expires in {formatCountdown(remainingMs)}
        </span>
      </div>
      <Button
        size="sm"

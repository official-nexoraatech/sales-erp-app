...
export const SEARCH_ENTITY_CONFIG: Record<string, SearchEntityConfig> = {
  customer: {
    ...
    icon: Users,
    route: (h) => `/customers/${h.id}`,
  },
  supplier: {
    label: 'Supplier',
    groupLabel: 'Suppliers',
    icon: Truck,
    route: (h) => `/suppliers/${h.id}/edit`,
  },
  item: {
    label: 'Item',
    groupLabel: 'Inventory',
    icon: Package,
    route: (h) => `/inventory/items/${h.id}/edit`,
  },
  category: {
    label: 'Category',
    groupLabel: 'Inventory',
  ...
  stock_transfer: {
    ...
    groupLabel: 'Inventory',
    icon: ArrowLeftRight,
    route: (h) => `/inventory/transfers/${h.id}`,
  },
  stock_adjustment: {
  ...
  invoice: {
    ...
    groupLabel: 'Sales',
    icon: FileText,
    route: (h) => `/sales/invoices/${h.id}`,
  },
  quotation: {
    label: 'Quotation',
    groupLabel: 'Sales',
    icon: FileText,
    route: (h) => `/sales/quotations/${h.id}`,
  },
  crm_interaction: {
    label: 'Interaction',
    groupLabel: 'CRM',
    icon: MessageSquare,
    route: (h) => (h.source['customerId'] ? `/customers/${h.source['customerId']}` : undefined),
  },
  ...
  account: {
    ...
    icon: Landmark,
    route: (h) => `/accounting/accounts/${h.id}/ledger`,
  },
...
export function getSearchResultSubtitle(hit: SearchHit): string | undefined {
  ...
  if (typeof s['amount'] === 'string' || typeof s['amount'] === 'number')
    parts.push(`₹${s['amount']}`);

...
// Phase 9 GUC-per-request rollout — migrated 2026-08-21. Previously deferred for time/effort
// economics (7 near-identical routes), not a real caveat blocker — no external I/O anywhere in
// this file. The lock route's ctx.db.transaction() call becomes a savepoint of the outer
// transaction once wrapped, and post-hoc ctx.audit.log() calls are safe per caveat 4b.
export async function openingBalancesRoutes(
  ...
  fastify.get(
    ...
    { preHandler: [authenticate, requirePermission(PERMISSIONS.OPENING_BALANCE_LOCK)] },
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      const [wizard] = await ctx.db.raw
        .select()
        .from(openingBalancesWizard)
        .where(eq(openingBalancesWizard.tenantId, ctx.tenant.tenantId));
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      if (wizard?.status === 'LOCKED') {
        throw new BusinessError(
          ...
          'Opening balances have been locked and cannot be modified'
        );
      }
      ...
      for (const row of body.data) {
        const [cust] = await ctx.db.raw
          .select()
          .from(customers)
          .where(
            and(eq(customers.id, row.customerId), eq(customers.tenantId, ctx.tenant.tenantId))
          );
      ...
      await ctx.db.raw
        ...
        .where(
          and(
            eq(openingBalances.tenantId, ctx.tenant.tenantId),
      ...
      await ctx.audit.log({
        ...
        metadata: { entitySubType: 'CUSTOMER', count: body.data.length },
      });

      return reply
        .code(200)
        .send({ data: { message: 'Customer opening balances saved', count: body.data.length } });
    })
  );
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      for (const row of body.data) {
        const [sup] = await ctx.db.raw
          .select()
          .from(suppliers)
          .where(
            and(eq(suppliers.id, row.supplierId), eq(suppliers.tenantId, ctx.tenant.tenantId))
          );
      ...
      await ctx.audit.log({
        action: 'CREATE',
        entityType: 'opening_balance',
        metadata: { entitySubType: 'SUPPLIER', count: body.data.length },
      });

      return reply
        .code(200)
        .send({ data: { message: 'Supplier opening balances saved', count: body.data.length } });
    })
  );
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      await ctx.db.raw
        ...
        .where(
          and(
            eq(openingBalances.tenantId, ctx.tenant.tenantId),
            eq(openingBalances.entityType, 'STOCK')
          )
        );
      ...
      await ctx.audit.log({
        action: 'CREATE',
        entityType: 'opening_balance',
        metadata: { entitySubType: 'STOCK', count: body.data.length },
      });

      return reply
        .code(200)
        .send({ data: { message: 'Stock opening balances saved', count: body.data.length } });
    })
  );
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      for (const row of body.data) {
        const [acc] = await ctx.db.raw
          .select()
          .from(accounts)
          .where(and(eq(accounts.id, row.accountId), eq(accounts.tenantId, ctx.tenant.tenantId)));
      ...
      await ctx.audit.log({
        action: 'CREATE',
        entityType: 'opening_balance',
        metadata: { entitySubType: 'ACCOUNT', count: body.data.length },
      });

      return reply
        .code(200)
        .send({ data: { message: 'Account opening balances saved', count: body.data.length } });
    })
  );
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      await ctx.db.raw
        ...
        .where(
          and(
            eq(openingBalances.tenantId, ctx.tenant.tenantId),
            eq(openingBalances.entityType, 'CASH_BANK')
          )
        );
      ...
      await ctx.audit.log({
        action: 'CREATE',
        entityType: 'opening_balance',
        metadata: { entitySubType: 'CASH_BANK', count: body.data.length },
      });

      return reply
        .code(200)
        .send({ data: { message: 'Cash/Bank opening balances saved', count: body.data.length } });
    })
  );
  ...
  fastify.post(
    ...
    tenantScopedHandler(ctxFactory, async (request, reply, ctx) => {
      ...
      if (!wizard)
        throw new BusinessError('NO_OPENING_BALANCES', 'No opening balances found to lock');
      if (wizard.status === 'LOCKED')
        throw new BusinessError('ALREADY_LOCKED', 'Opening balances are already locked');
      ...
      const allBalances = await ctx.db.raw
        ...
        .where(eq(openingBalances.tenantId, ctx.tenant.tenantId));
          ...
          ? await ctx.db.raw
              ...
              .where(
                and(
                  ...
                  inArray(accounts.id, accountEntityIds)
                )
              )
          : [];
      ...
      await ctx.db.transaction(async (trx) => {
        ...
        await eventBus.publishInTransaction(
          ...
          ctx.tenant.tenantId,
          'OPENING_BALANCES_LOCKED',
          { tenantId: ctx.tenant.tenantId, lockedAt, totalDebit, totalCredit }
        );
      });
      await ctx.audit.log({
        action: 'UPDATE',

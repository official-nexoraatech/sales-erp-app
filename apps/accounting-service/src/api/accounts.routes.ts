...
export async function accountRoutes(
  ...
  fastify.post('/accounts', { preHandler: [authenticate, requirePermission(PERMISSIONS.ACCOUNT_CREATE)] }, async (request, reply) => {
    ...
    const created = await ctx.db.transaction(async (trx) => {
      ...
      await eventBus.publishInTransaction('account', row.id, 'ACCOUNT_CREATED', row as unknown as Record<string, unknown>);
      return row;
    });
  ...
  fastify.put<{ Params: { id: string } }>('/accounts/:id', { preHandler: [authenticate, requirePermission(PERMISSIONS.ACCOUNT_UPDATE)] }, async (request, reply) => {
    ...
    const updated = await ctx.db.transaction(async (trx) => {
      ...
      const eventBus = new PlatformEventBus(trx, tenantId, userId, ctx.tenant.correlationId);
      await eventBus.publishInTransaction('account', row.id, 'ACCOUNT_UPDATED', row as unknown as Record<string, unknown>);
      return row;
    });
  ...
  fastify.delete<{ Params: { id: string } }>('/accounts/:id', { preHandler: [authenticate, requirePermission(PERMISSIONS.ACCOUNT_UPDATE)] }, async (request, reply) => {
    ...
    const [hasEntries] = await ctx.db.raw
      ...
      .limit(1);
    if (hasEntries) {
      throw new BusinessError('ACCOUNT_HAS_TRANSACTIONS', 'Cannot delete an account that has posted financial entries');
    }
  ...
  fastify.post('/accounts/seed', { preHandler: [authenticate, requirePermission(PERMISSIONS.ACCOUNT_CREATE)] }, async (request, reply) => {
    ...
    await ctx.db.transaction(async (trx) => {
      ...
      const existingRows = await trx.raw.select().from(accounts).where(eq(accounts.tenantId, tenantId));
      ...
      const eventBus = new PlatformEventBus(trx, tenantId, userId, ctx.tenant.correlationId);
      await eventBus.publishInTransaction('account', tenantId, 'CHART_OF_ACCOUNTS_SEEDED', { tenantId, count: DEFAULT_ACCOUNTS.length });
    });

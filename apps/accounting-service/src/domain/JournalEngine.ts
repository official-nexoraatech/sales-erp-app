...
export class JournalEngine {
  ...
  static async post(
    ...
    if (entry.lines.length < 2) {
      throw new BusinessError('JOURNAL_INSUFFICIENT_LINES', 'A journal entry requires at least 2 lines');
    }
    ...
    const foundAccounts = await db.raw
      .select({ id: accounts.id, accountCode: accounts.accountCode, name: accounts.name, isActive: accounts.isActive })
      .from(accounts)
    ...
    for (const id of accountIds) {
      ...
      if (!acc.isActive) {
        throw new BusinessError('ACCOUNT_INACTIVE', `Account ${id} is inactive and cannot receive postings`);
      }
    }
  ...
  static async reverse(
    ...
    if (originalJournal.status === 'REVERSED') {
      throw new BusinessError('JOURNAL_ALREADY_REVERSED', `Journal ${originalJournalId} is already reversed`);
    }
    ...
    const originalEntries = await db.raw
      ...
      .where(and(eq(financialEntries.tenantId, tenantId), eq(financialEntries.journalId, originalJournalId)));

    if (originalEntries.length === 0) {
      throw new BusinessError('JOURNAL_NO_ENTRIES', `Journal ${originalJournalId} has no entries to reverse`);
    }

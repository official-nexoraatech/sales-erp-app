...
export class ReportsEngine {
  ...
  static async getProfitLoss(
    ...
    const totalCogs = cogs.reduce((s, l) => s + l.amount, 0) + contraRevenue.reduce((s, l) => s + l.amount, 0);
  ...
  static async getCashFlow(
    ...
    // Cash & bank accounts (CASH_AND_BANK sub-type)
    const cashRows = await db.raw.execute(sql`
      SELECT
        COALESCE(SUM(fe.debit_amount), 0)::NUMERIC  AS total_in,
        COALESCE(SUM(fe.credit_amount), 0)::NUMERIC AS total_out,
        a.name
      FROM accounts a
      JOIN financial_entries fe
        ON fe.account_id = a.id
      ...
      WHERE a.tenant_id      = ${tenantId}
        ...
        AND a.deleted_at IS NULL
      GROUP BY a.name
    `) as Array<{ total_in: string; total_out: string; name: string }>;
    ...
    const totalCashIn = cashRows.reduce((s, r) => s + Number(r.total_in), 0);
    const totalCashOut = cashRows.reduce((s, r) => s + Number(r.total_out), 0);

...
import { QuotationService, type QuotationLineInput } from './QuotationService.js';
...
export class OpportunityService {
  ...
  /**
   * Marks an opportunity Won and — per AR-2 — reuses QuotationService.create() rather than
   * duplicating quotation-creation logic. Line items carry no GST/HSN (see
   * crmOpportunityLineItems' own comment); both are looked up fresh from `items` here, exactly
   * as they would be if a rep created the quotation manually today.
   */
  /**
   * Wrapped in one transaction — per this feature's own testing requirement, the stage
   * transition and the quotation it creates must be a single atomic operation, not two
   * separately-committed steps that could leave a Won opportunity with no quotation if the
   * second step failed. `QuotationService.create()` opens its own nested transaction
   * internally; running it against `trx` here makes that a savepoint within this one, not a
   * second independently-committing transaction.
   */
  static async markWon(
  ...
  ): Promise<{ opportunity: typeof crmOpportunities.$inferSelect; quotationId: number }> {
    return db.transaction(async (trxRaw) => {
      ...
      await OpportunityService.changeStage(trx, tenantId, userId, opportunityId, {
      ...
      const quotationService = new QuotationService(trx);
      ...
      const [withQuotation] = await trx
        .update(crmOpportunities)
        .set({ convertedQuotationId: quotationId })
        .where(eq(crmOpportunities.id, opportunityId))
        .returning();
      if (!withQuotation) throw new Error('Opportunity update after Won failed unexpectedly');

      return { opportunity: withQuotation, quotationId };
    });
  }

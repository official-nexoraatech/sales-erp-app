...
// CRM/O2C split — this is half of a split class. segmentCounts, getPredictionsForCustomer,
// and recordFeedback moved to apps/crm-service/src/domain/HealthScoringService.ts because
// they only read/write the cached crmChurnPredictions/crmNextBestActions/
// crmProductRecommendations/customers.healthSegment tables. What stays here (computeForTenant,
// scoreCustomer, predictChurn, computeProductRecommendations, computeAndCachePredictions) is
// genuinely O2C-coupled — it reads invoices/payments/sale_returns/invoice_lines directly via
// raw SQL — and writes its results into those same CRM-owned tables via a direct cross-service
// Postgres write (documented exception, mirrors the shared-table read precedent already used
// for customers/branches in earlier CRM/O2C migrations).

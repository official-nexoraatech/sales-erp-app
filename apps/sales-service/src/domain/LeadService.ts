...
export class LeadService {
   ...
   * Feature 1), reusing CustomerService.create — never duplicates customer-creation logic. If
   * a non-deleted customer with the same phone already exists, the lead attaches to that
   * existing customer instead of creating a duplicate ("offer merge into existing", not a
   * blind duplicate — the feature's stated edge case).
   */
  static async convertToCustomer(
    ...
    if (existingCustomer) {
      ...
      const { created } = await CustomerService.create(db, {
        tenantId,
        createdBy: userId,

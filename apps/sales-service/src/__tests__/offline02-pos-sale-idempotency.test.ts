...
describe('POST /pos/sales — OFFLINE-02 idempotent retry handling', () => {
  ...
  it('returns the already-committed original result on a retried operationId, without calling confirm()', async () => {
    ...
    const script = [
      ...
      [
        {
          id: 42,
          invoiceNumber: 'POS-1-100',
          status: 'CONFIRMED',
          grandTotal: '118.00',
          loyaltyPointsEarned: 0,
          loyaltyRedemptionValue: '0.00',
        },
      ], // waitForOperationResult poll #1 — already confirmed
    ...
    const body = JSON.parse(res.body) as {
      data: { invoiceId: number; invoiceNumber: string; paymentIds: number[] };
    };
  ...
  it('polls until the in-flight winner commits, then returns its result', async () => {
    ...
    const script = [
      ...
      ], // poll #1 — winner still mid-flight
      [
        {
          id: 42,
          invoiceNumber: 'POS-1-100',

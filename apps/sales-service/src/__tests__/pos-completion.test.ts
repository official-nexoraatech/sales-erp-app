...
// optOutCondition and the CampaignService.resolveRecipients opt-out-exclusion scenario moved to
// crm-service's campaign-service.test.ts (CRM/O2C split, migration 7) — this file's own copies
// were fully redundant with that dedicated CampaignService test file's own coverage of both.
...
describe.skipIf(!DB_URL)('POS completion — integration', () => {
  ...
  it('recording and allocating a POS payment reduces the invoice balance to zero — the core POS payment-recording fix', async () => {
    const [customer] = await db
      ...
      .values({
        tenantId: TEST_TENANT,
        branchId,
        displayName: 'POS Payment Customer',
        phone: '9000000020',
        creditLimit: '0',
        openingBalance: '0',
        createdBy: 1,
    ...
    const paymentId = await paymentSvc.create({
      ...
      branchId,
      customerId: customer!.id,
      paymentNumber: `PAY-TEST-${invoice!.id}`,
  ...
  it('splitting a payment across two modes (e.g. cash + card) still fully allocates the invoice', async () => {
    const [customer] = await db
      ...
      .values({
        tenantId: TEST_TENANT,
        branchId,
        displayName: 'Split Payment Customer',
        phone: '9000000021',
        creditLimit: '0',
        openingBalance: '0',
        createdBy: 1,
    ...
    for (const [mode, amount] of [
      ...
      const paymentId = await paymentSvc.create({
        ...
        branchId,
        customerId: customer!.id,
        paymentNumber: `PAY-TEST-SPLIT-${invoice!.id}-${mode}`,
        paymentDate: new Date(),
        paymentMode: mode,
        amount,
        createdBy: 1,
  ...
  it('rejects redeeming more points than the customer has', async () => {
    const [customer] = await db
      ...
      .values({
        tenantId: TEST_TENANT,
        branchId,
        displayName: 'Insufficient Points Customer',
        phone: '9000000023',
        creditLimit: '0',
        openingBalance: '0',
        createdBy: 1,
    ...
    const loyaltySvc = new LoyaltyService(db);
    await expect(
      loyaltySvc.redeemPoints(TEST_TENANT, customer!.id, 100, 'POS_SALE', 1, 1)
    ).rejects.toThrow('Only 5 points available');
  });
  ...
  it('hold and resume round-trip preserves the parked cart', async () => {
    ...
    const [reloaded] = await db
      .select()
      .from(posHeldSales)
      .where(and(eq(posHeldSales.id, held!.id), eq(posHeldSales.tenantId, TEST_TENANT)));

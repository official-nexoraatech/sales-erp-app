  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const [leadFunnel, ticketSla, campaignPerformance, quotaAttainmentRaw] = await Promise.all([
        canViewLeads
          ? CrmDashboardService.getLeadFunnel(ctx.db.raw, tenantId, branchScope, range)
          : Promise.resolve(null),
        canViewTickets
          ? CrmDashboardService.getTicketSlaCompliance(ctx.db.raw, tenantId, branchScope, range)
          : Promise.resolve(null),
        canViewCampaigns
          ? CrmDashboardService.getCampaignPerformance(ctx.db.raw, tenantId, branchScope, range)
          : Promise.resolve(null),
        canViewQuotas
          ? CrmDashboardService.getQuotaAttainment(ctx.db.raw, tenantId, periodYear, periodMonth)
          : Promise.resolve(null),
      ]);

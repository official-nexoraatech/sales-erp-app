...
// instead of `tenantScopedHandler`, matching accounting-service's copy of this same file pattern.
      ...
      if (!KNOWN_ENTITIES.includes(entity)) {
        return reply
          ...
          .send({
            error: {
              code: 'INVALID_ENTITY',
              message: `sales-service does not own entity: ${entity}`,
            },
          });
      }
      ...
      const content: SearchSyncDoc[] = await withTenantConnection(
        ...
        async (db) => {
          if (entity === 'customer') {
            ...
            return rows.map((r) => ({
              id: String(r.id),
              doc: { name: r.name, channel: r.channel, status: r.status, tenantId },
            }));
          } else {
            // entity === 'payment' — the only remaining member of KNOWN_ENTITIES.

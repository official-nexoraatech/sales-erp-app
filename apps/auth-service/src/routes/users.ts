...
import { BusinessError, ERPError, NotFoundError, PermissionError, ValidationError } from '@erp/types';
...
function sanitizeUser(user: User): Omit<User, 'passwordHash' | 'totpSecret' | 'backupCodes'> {
...
const ResetPasswordSchema = z.object({
  currentPassword: z.string().min(1, "Admin's current password is required to reset another user's password"),
  newPassword: z.string().min(12, 'Password must be at least 12 characters'),
});

type AuthedRequest = { auth: { tenantId: number; userId: number; permissions: string[] } };

export async function userRoutes(fastify: FastifyInstance, ctxFactory: PlatformContextFactory): Promise<void> {
  ...
  fastify.get('/users', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_VIEW)] }, async (request, reply) => {
  ...
  fastify.get<{ Params: { id: string } }>('/users/:id', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_VIEW)] }, async (request, reply) => {
    const { tenantId } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    const id = parseInt(request.params.id, 10);

    const [user] = await ctx.db.raw.select().from(users).where(and(eq(users.id, id), eq(users.tenantId, tenantId)));
    if (!user) throw new NotFoundError('User', id);

    const userRoleRows = await ctx.db.raw.select().from(userRoles).where(eq(userRoles.userId, id));
    const branchRows = await ctx.db.raw.select().from(userBranches).where(eq(userBranches.userId, id));
  ...
  fastify.post('/users', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_CREATE)] }, async (request, reply) => {
    const { tenantId, userId, permissions: callerPermissions } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    ...
    await ctx.events.publish('user', newUser.id, 'USER_CREATED', safeUser as unknown as Record<string, unknown>);
    return reply.code(201).send({ data: { ...safeUser, createdBy: userId } });
  });

  // ── PUT /users/:id — Update user ──────────────────────────────────────
  fastify.put<{ Params: { id: string } }>('/users/:id', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_UPDATE)] }, async (request, reply) => {
    const { tenantId } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    const id = parseInt(request.params.id, 10);
    ...
    const [existing] = await ctx.db.raw.select().from(users).where(and(eq(users.id, id), eq(users.tenantId, tenantId)));
    ...
    await ctx.events.publish('user', id, 'USER_UPDATED', safeUser as unknown as Record<string, unknown>);
    return reply.code(200).send({ data: safeUser });
  });

  // ── DELETE /users/:id — Soft delete (mark inactive, cannot delete last OWNER) ──
  fastify.delete<{ Params: { id: string } }>('/users/:id', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_DELETE)] }, async (request, reply) => {
    const { tenantId } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    const id = parseInt(request.params.id, 10);
    ...
    const ownerRole = await ctx.db.raw.select().from(roles).where(and(eq(roles.tenantId, tenantId), eq(roles.name, 'OWNER')));
    ...
    await ctx.db.raw.update(users).set({ isActive: false, updatedAt: new Date() }).where(eq(users.id, id));
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const [caller] = await ctx.db.raw.select().from(users).where(and(eq(users.id, callerId), eq(users.tenantId, tenantId)));
      if (!caller) throw new NotFoundError('User', callerId);
      const callerPasswordValid = await argon2.verify(caller.passwordHash, body.data.currentPassword);
      ...
      await ctx.db.raw.update(users).set({ passwordHash, failedLoginAttempts: 0, lockedUntil: null, updatedAt: new Date() }).where(eq(users.id, id));
  ...
  // ── POST /users/:id/lock ───────────────────────────────────────────────
  fastify.post<{ Params: { id: string } }>('/users/:id/lock', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_MANAGE)] }, async (request, reply) => {
    const { tenantId } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    const id = parseInt(request.params.id, 10);
    const [existing] = await ctx.db.raw.select().from(users).where(and(eq(users.id, id), eq(users.tenantId, tenantId)));
    ...
    await ctx.db.raw.update(users).set({ lockedUntil: lockUntil, updatedAt: new Date() }).where(eq(users.id, id));
    return reply.code(200).send({ data: { message: 'User locked', id } });
  });

  // ── POST /users/:id/unlock ────────────────────────────────────────────
  fastify.post<{ Params: { id: string } }>('/users/:id/unlock', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_MANAGE)] }, async (request, reply) => {
    const { tenantId } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    const id = parseInt(request.params.id, 10);
    const [existing] = await ctx.db.raw.select().from(users).where(and(eq(users.id, id), eq(users.tenantId, tenantId)));
    if (!existing) throw new NotFoundError('User', id);
    await ctx.db.raw.update(users).set({ lockedUntil: null, failedLoginAttempts: 0, updatedAt: new Date() }).where(eq(users.id, id));
    return reply.code(200).send({ data: { message: 'User unlocked', id } });
  });

  // ── PUT /users/:id/branches — Assign branch access ───────────────────
  fastify.put<{ Params: { id: string } }>('/users/:id/branches', { preHandler: [authenticate, requirePermission(PERMISSIONS.USER_MANAGE)] }, async (request, reply) => {
    const { tenantId } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    const id = parseInt(request.params.id, 10);
    ...
    await ctx.db.raw.delete(userBranches).where(and(eq(userBranches.userId, id), eq(userBranches.tenantId, tenantId)));
    if (body.data.branchIds.length > 0) {
      await ctx.db.raw.insert(userBranches).values(
        body.data.branchIds.map((branchId) => ({
          userId: id,
  ...
  fastify.get('/users/me', async (request, reply) => {
    const { tenantId, userId } = (request as unknown as AuthedRequest).auth;
    const ctx = ctxFor(request);
    const [user] = await ctx.db.raw.select().from(users).where(and(eq(users.id, userId), eq(users.tenantId, tenantId)));
    if (!user) throw new NotFoundError('User', userId);
    const userRoleRows = await ctx.db.raw.select().from(userRoles).where(eq(userRoles.userId, userId));
    const branchRows = await ctx.db.raw.select().from(userBranches).where(eq(userBranches.userId, userId));
    const safeUser = sanitizeUser(user);
    return reply.code(200).send({ data: { ...safeUser, roleIds: userRoleRows.map((r) => r.roleId), branches: branchRows } });
  });
  ...
  fastify.put('/users/me', async (request, reply) => {
    ...
    if (!body.success) throw new ValidationError(body.error.errors.map((e) => e.message).join('; '));
    const [existing] = await ctx.db.raw.select().from(users).where(and(eq(users.id, userId), eq(users.tenantId, tenantId)));
    ...
    if (!updated) throw new Error('User update failed unexpectedly');
    const safeUser = sanitizeUser(updated);
    await ctx.events.publish('user', userId, 'USER_UPDATED', safeUser as unknown as Record<string, unknown>);
    return reply.code(200).send({ data: safeUser });
  });
  ...
  fastify.put('/users/me/password', async (request, reply) => {
    ...
    const passwordHash = await argon2.hash(body.data.newPassword, { type: argon2.argon2id });
    await ctx.db.raw.update(users).set({ passwordHash, updatedAt: new Date() }).where(eq(users.id, userId));

...
> **RESOLVED (core scope) — confirmed by the 2026-07-22 Tenant Service enterprise audit** (`ERP-PLANNING/phase-completions/TENANT-SERVICE-ENTERPRISE-AUDIT-2026-07-22.md`). `TenantProvisioner`'s `CONFIGURE_S3` step now performs a real `bucketExists()` check followed by a real `uploadFile()` call, is fatal on failure (`provisioningStatus: 'FAILED'`, re-thrown `S3_PROVISIONING_FAILED`), matching this doc's Architecture/Acceptance Criteria. The one item this doc explicitly flagged as a possible follow-up — per-tenant IAM bucket policies / distinct MinIO credentials per tenant (isolation is still path-convention-only, `tenants/${tenantId}/...`) — remains genuinely unimplemented and is the only part of this doc still open. Kept below for historical context; do not re-implement the core bootstrap.

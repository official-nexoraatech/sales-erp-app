...
This package is primarily a **plan document + defensive code changes**, not a full re-platforming — actually standing up a multi-node ES cluster is an infra/ops task (parallel to PG-022 Kubernetes readiness), not something this package should attempt to implement blind. What this package _should_ produce:
...
- [x] `createIndex()` and `fullReindex()` in `SearchEngine.ts` explicitly set `number_of_shards`/`number_of_replicas` (verifiable by reading the ES `PUT /{index}` request body sent in a test, not relying on ES cluster defaults). Done 2026-07-11 — `number_of_shards: 1` was already explicit; `number_of_replicas` dropped from `1` to `0` per the "Performance" section's recommendation. See `IMPLEMENTATION-NOTES.md`'s "PG-049" entry for a discovered second, undocumented index-creation path (`tenant-service`'s `TenantProvisioner.createEsIndices()`) with a real name-mismatch bug, left unfixed (out of scope).

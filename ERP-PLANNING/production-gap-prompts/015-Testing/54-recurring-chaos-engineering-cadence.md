...
- [ ] A documented quarterly schedule exists (first Monday of Q1/Q2/Q3/Q4, matching the DR-drill report's own recommendation) with a concrete next-due date.
- [ ] The next chaos run's planned calendar explicitly includes a new Experiment 3.2b (caller-level idempotency-dedup re-test for the MSG91/notification retry path) in addition to re-running the original 9 experiments.
- [ ] A report-file naming convention is documented and the original `chaos-engineering-report.md` is left untouched as the historical Q3-2026 baseline.
- [ ] A written process step confirms: any new finding from a chaos run becomes its own `PG-XXX` file under `production-gap-prompts/`, following `_TEMPLATE.md`, rather than staying only inside the dated report.
- [ ] (If built) the optional scheduled reminder workflow successfully opens a tracking issue on a test/dry-run trigger.

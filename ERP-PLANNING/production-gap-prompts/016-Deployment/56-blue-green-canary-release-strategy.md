...
- **What already exists and should be reused:**
  ...
  - `infrastructure/istio/` — **inventoried 2026-07-11, and it's a third outcome, not the binary the original text anticipated.** It contains two real, non-trivial manifests (`peer-authentication.yaml`: namespace-wide `PeerAuthentication` set to `STRICT` mTLS, plus a `PERMISSIVE` port-9090 carve-out for Prometheus scraping; `authorization-policy.yaml`: a default-deny `AuthorizationPolicy` plus explicit allow-rules for API-gateway→auth-service, health-check GETs, and Prometheus scraping) — this is not `.gitkeep`-only scaffolding like `infrastructure/helm/` was. But per the Helm chart's own README (`infrastructure/helm/erp/README.md`, "Known gaps"): **no Istio control-plane install exists anywhere in this repo's IaC**, so these mesh-level policies are not actually enforced today — Kubernetes `NetworkPolicy` is the only network segmentation actually in effect. And critically for _this_ package: there is **zero traffic-splitting config** (`VirtualService`/`DestinationRule`/`Gateway`) anywhere in `infrastructure/` — confirmed by grep. So Istio here is real-but-inert for the security purpose it was written for, and offers this package's canary path no head start at all; a canary implementation would need to build 100% of the traffic-splitting layer from scratch (and separately decide whether to actually install the Istio control plane, a decision PG-022 Session 3 explicitly deferred, not this package).
  - PG-057's rollback procedure (application-only `kubectl rollout undo` as the default path) — blue/green's rollback is actually _simpler_ than PG-057's general case in one respect (the old "blue" environment is still fully running and can be instantly re-pointed-to), but _harder_ in the schema-compatibility respect described above, since it must hold for the entire overlap window, not just the moment of cutover.
...
- Not applicable as a schema change from this package itself — but this package's binding constraint (see Architecture) means every _future_ migration, for as long as blue/green/canary releases are in use, must additionally satisfy "safe for both the currently-live old version and the incoming new version to query simultaneously for the full overlap window," which is a stricter, ongoing discipline this package would impose on `packages/db-client/migrations` going forward, not a one-time migration this package itself adds.

## Backend

Not applicable — no application code changes from this package itself (aside from imposing the stricter dual-version schema-compatibility discipline on all _future_ migrations, which is a process constraint, not a code change).
...
- The core multi-tenant fact governing this entire package: because tenant isolation here is application-code `tenant_id` filtering against one shared database (not per-tenant schema/instance), there is no way to migrate one tenant's traffic to the "green"/canary version while others stay on "blue" at the database level — a canary-by-tenant-cohort approach (a genuinely useful lower-risk pattern for a multi-tenant SaaS) is possible at the _application-routing_ layer (e.g., route specific pilot tenants' requests to the canary Deployment) but does **not** reduce the shared-schema compatibility constraint at all, since all tenants' data lives in the same tables regardless of which app version serves which tenant's requests.
...
- [x] `infrastructure/istio/`'s actual contents are inventoried and documented (wired and real, or scaffold-only like `infrastructure/helm/` was found to be) so a future session doesn't have to re-discover this. **Verified 2026-07-11: neither binary option — see updated Existing Code Analysis bullet below.**
- [x] A documented recommendation exists (blue/green over canary, per the Architecture section's reasoning) so a future session doesn't re-litigate the choice from scratch.
- [x] This package remains explicitly filed under Phase 9 (Enterprise Enhancements), not promoted to Phase 8, unless a concrete customer/business driver is named that requires it — that decision itself is the actual, checkable deliverable of this package as scoped today. **Re-confirmed 2026-07-11: no such driver has been named.**
- [x] No blue/green/canary infrastructure is built prematurely — confirmed by this package's own Deliverables being limited to documentation/evaluation, not `infrastructure/k8s` manifest changes.
...
**Previous Work Summary:** No blue/green or canary deployment infrastructure exists. PG-057 (this package's dependency) **shipped 2026-07-11** — `ERP-PLANNING/runbooks/production-deployment-runbook.md` establishes an ordinary rolling-deploy-plus-documented-rollback runbook (application-only `kubectl rollout undo` as default, DR restore as exception path), judged sufficient for this system's actual target market (SMB-tier ERP, short-maintenance-window-tolerant); its own Deployment Checklist notes the staging dry-run and a real second cluster/production KUBECONFIG are still not provisioned. `infrastructure/istio/` was inventoried 2026-07-11: it holds real, wired mTLS/authorization-policy manifests, but no Istio control plane is installed anywhere in this repo's IaC and there is no traffic-splitting config (`VirtualService`/`DestinationRule`/`Gateway`) at all — see the updated Existing Code Analysis bullet.

**Current Objective:** This package's actual current deliverable is a _decision_, not an implementation: document that a shared, single-Postgres, no-per-tenant-schema architecture means any zero-downtime strategy requires strict dual-version schema compatibility for the full cutover window regardless of which strategy is picked; recommend blue/green over canary for this system's specific shape (14 services, no existing traffic-shaping infra, business-hours usage pattern); and confirm this stays a Phase 9 enterprise-enhancement item, not a Phase 8 production-readiness requirement, unless a specific customer commitment says otherwise.
...
2. `infrastructure/istio/` inventoried 2026-07-11: real mTLS + AuthorizationPolicy manifests exist, but no Istio control plane is installed anywhere and there is no traffic-splitting config — treat it as offering zero head start for a canary implementation.
3. `infrastructure/helm/` is **no longer empty** — PG-022 Session 1 landed 2026-07-10 (Chart.yaml + Deployment/Service/HPA/PDB/ServiceAccount templates for all 14 services). It is not yet wired into CI, though: `deploy-staging` still runs `kubectl apply -k infrastructure/k8s/` against the flat Kustomize manifests (confirmed via PG-057's completion report). Ingress/Gateway, NetworkPolicy extension beyond `auth-service`, and the Istio install-or-descope decision are explicitly PG-022 Session 2/3, not done.
4. PG-057 (this package's dependency) **shipped 2026-07-11** — the actual Phase 8 production-readiness deliverable is in place; this package remains explicitly deferred past it into Phase 9.
...
**Known Constraints:** No live cluster needed to complete this package's actual current scope (a decision document); if a future session is asked to _implement_ blue/green, it should re-confirm PG-021/PG-022's actual landed state first, since this package assumes both are done.
...
**Feature Flags:** the existing tenant feature-flag system gates _product features_ per tenant, not _release versions_ per request — do not conflate the two if a future session considers tenant-cohort canarying.

**Multi-Tenant Rules:** tenant isolation is shared-database/application-code only — no mechanism exists to give a canary/green version its own data copy; any tenant-cohort canary approach would only shift _routing_, never _data isolation_.

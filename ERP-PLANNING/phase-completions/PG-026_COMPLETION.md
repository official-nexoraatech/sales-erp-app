...
- [x] **Follow up on the `workflowApprovals.approverId`-is-a-role-id bug** (see dedicated section above) — a real, separate, larger fix needed before "pending approvals for me" or any per-approver notification can be trusted. Fixed 2026-07-17: `WorkflowEngine.resolveApprovers` (`packages/platform-sdk/src/workflow.ts`) now resolves ROLE nodes to every active user holding that role via `userRoles`, creating one approval row per eligible approver (`approverRoleId` now populated too) instead of storing the role's own id. `getPendingForApprover`/`getStatus` now also scope "pending" to the approval's node matching the instance's current node, and `requireAllApprovers` nodes correctly wait for every eligible approver before advancing. Verified end-to-end against a live DB in `packages/platform-sdk/src/__tests__/workflow.test.ts` (3 new tests, all passing). No DB migration needed — this was pure application logic; existing `workflow_approvals` rows created under the old (broken) resolution remain unreachable dead rows for in-flight instances only, new triggers are correct immediately.
